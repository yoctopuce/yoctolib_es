    ...
    _processMore(newupdate) {
        ...
        return _asyncToGenerator(function* () {
            ...
            try {
                ...
                _this33.imm_progress(-1, e.message);
                _this33._yapi._throw(YAPI.IO_ERROR, e.message, null);
                return;
            }
        ...
        return _asyncToGenerator(function* () {
            ...
            if (use_self_flash) {
                ...
                let checkFlash = function () {
                    _this154.request('GET', baseUrl + '/flash.json?a=state', null, 1).then(function (flashReq) {
...
class YAPIContext {
    ...
    UpdateDeviceList(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            var yreq = yield _this191._updateDeviceList_internal(false, true);
            if (yreq.errorType != YAPI_SUCCESS) {
    ...
    HandleEvents(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            var nEvents = _this192._data_events.length;
            for (var i = 0; i < nEvents; i++) {
                var evt = _this192._data_events[i];
                if (typeof evt[1] == 'string') {
                    ...
                    var dev = _this192.imm_getDevice(evt[0]._serial);
                    if (dev) {
            ...
            _this192._data_events = _this192._data_events.slice(nEvents);
    ...
    Sleep(ms_duration, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            var end = _this193.GetTickCount() + ms_duration;
            yield _this193.HandleEvents(errmsg);
            while (_this193.GetTickCount() < end) {
                yield _this193._microSleep_internal();
                yield _this193.HandleEvents(errmsg);
            }
            return YAPI_SUCCESS;
        })();
    ...
    _setTimeout_internal(callback, endtime, args, errmsg) {
        ...
        if (delay < 0) {
            ...
            this.Sleep(delay, errmsg).then(function () {
                _this194._setTimeout_internal(callback, endtime, args);
            });
        } else {
            this.UpdateDeviceList(errmsg).then(function () {
                _this194.Sleep(90, errmsg).then(function () {
                    _this194._setTimeout_internal(callback, endtime, args);
                });
            });
        }
    ...
    RegisterDeviceArrivalCallback(arrivalCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this195._arrivalCallback = arrivalCallback;
        })();
    }
    ...
    RegisterDeviceChangeCallback(changeCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this196._namechgCallback = changeCallback;
        })();
    }
    ...
    RegisterDeviceRemovalCallback(removalCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this197._removalCallback = removalCallback;
        })();
    }
    ...
    RegisterCalibrationHandler(calibrationType, calibrationHandler) {
        ...
        return _asyncToGenerator(function* () {
            _this198._calibHandlers[calibrationType.toString()] = calibrationHandler;
        })();
    }
    ...
    ComputePSK(ssid, pass) {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this199.imm_initshaw(pass, 0, 0, 0x3636, _shaw);
            _this199.imm_itershaw(sha1_init, _shaw);
            for (k = 0; k < 5; k++) inner[k] = _shaw[k];
            _shaw = new Uint32Array(80);
            _this199.imm_initshaw(pass, 0, 0, 0x5c5c, _shaw);
            _this199.imm_itershaw(sha1_init, _shaw);
            ...
            _shaw = new Uint32Array(80);
            _this199.imm_initshaw(ssid, 0, 1, 0, _shaw);

            for (iter = 0; iter < 8192;) {
                _this199.imm_itershaw(inner, _shaw);
                _shaw[5] = 0x80000000;
                for (k = 6; k < 15; k++) {
                    _shaw[k] = 0;
                }
                _shaw[15] = 8 * (64 + 20);
                _this199.imm_itershaw(outer, _shaw);
                shau[0] ^= _shaw[0];
                shau[1] ^= _shaw[1];
                shau[2] ^= _shaw[2];
                shau[3] ^= _shaw[3];
                ...
                if ((iter & 4095) == 0) {
                    ...
                    if (iter == 4096) {
                        for (k = 0; k < 5; k++) shau[k] = 0;
                        _shaw = new Uint32Array(80);
                        _this199.imm_initshaw(ssid, 0, 2, 0, _shaw);
                    }
                }
            }
            var hex = '';
