/*********************************************************************
 *
 * $Id: yocto_api.js 26369 2017-01-13 09:28:17Z mvuilleu $
 *
...
class YDevice {
    ...
    updateFromReq(yreq, loadval) {
        ...
        return _asyncToGenerator(function* () {
            _this31._cache._expiration = _this31._yapi.GetTickCount() + _this31._yapi.defaultCacheValidity;
            _this31._cache._json = yreq.bin_result;

            var func;
            var reindex = false;
            if (_this31._productName == '') {
                // parse module and function names for the first time
                for (func in loadval) {
                    if (func == 'module') {
                        _this31._serialNumber = loadval.module.serialNumber;
                        _this31._logicalName = loadval.module.logicalName;
                        _this31._productName = loadval.module.productName;
                        _this31._productId = loadval.module.productId;
                        _this31._beacon = loadval.module.beacon;
                    } else if (func == 'services') {
                        _this31.imm_updateFromYP(loadval.services.yellowPages);
                    }
                }
                reindex = true;
            } else {
                ...
                for (func in loadval) {
                    if (func == 'module') {
                        if (_this31._logicalName != loadval.module.logicalName) {
                            _this31._logicalName = loadval.module.logicalName;
                            reindex = true;
                        }
                        _this31._beacon = loadval.module.beacon;
                        ...
                        if (pubval != undefined) {
                            yield _this31._yapi.setFunctionValue(loadval.module.serialNumber + '.' + func, pubval);
                        }
                        var funydx;
                        for (funydx in _this31._functions) {
                            if (_this31._functions[funydx][0] == func) {
                                if (_this31._functions[funydx][1] != name) {
                                    _this31._functions[funydx][1] = name;
                                    reindex = true;
                                }
                                break;
                            }
            ...
            if (reindex) {
                _this31._yapi.imm_reindexDevice(_this31);
            }
        })();
    }
    ...
    requestAPI(int_msValidity) {
        ...
        return _asyncToGenerator(function* () {
            if (_this32._cache._expiration > _this32._yapi.GetTickCount()) {
                return new YHTTPRequest(_this32._cache._json);
            }
            /** @type {YHTTPRequest} **/
            let yreq = yield _this32._yapi.devRequest(_this32._rootUrl, 'GET /api.json', null, 0);
            if (yreq.errorType != YAPI_SUCCESS) return yreq;
            if (!int_msValidity) {
                int_msValidity = _this32._yapi.defaultCacheValidity;
            }
            _this32._cache._expiration = _this32._yapi.GetTickCount() + int_msValidity;
            _this32._cache._json = yreq.bin_result;
            return yreq;
        })();
    }
    ...
    refresh() {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YHTTPRequest} **/
            let yreq = yield _this33.requestAPI(_this33._yapi.defaultCacheValidity);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this33._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }

            let loadval = null;
            try {
                loadval = JSON.parse(_this33._yapi.imm_bin2str(yreq.bin_result));
            } catch (err) {}
            if (!loadval) {
                return _this33._throw(YAPI_IO_ERROR, 'Request failed, could not parse API result for ' + _this33._rootUrl, YAPI_IO_ERROR);
            }
            yield _this33.updateFromReq(yreq, loadval);
            return YAPI_SUCCESS;
        })();
    }
}
...
class YFirmwareUpdate {
    ...
    _processMore(newupdate) {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this34.imm_progress(0, 'Firmware update started');
            if (typeof _this34._firmwarepath == 'string' && _this34._firmwarepath.indexOf('yoctopuce.com') >= 0) {
                _this34.imm_progress(1, 'Downloading firmware');
                bytes = yield YFirmwareUpdate._downloadfile(_this34._firmwarepath);
            } else {
                _this34.imm_progress(1, 'Loading firmware');
                bytes = yield YFirmwareUpdate._loadfile(_this34._firmwarepath);
            }
            /** @type {YFirmwareFile} **/
            let firmware = YFirmwareFile.imm_Parse(_this34._firmwarepath, bytes, _this34._force);

            //5% -> 10%
            _this34.imm_progress(5, 'Check if module is already in bootloader');
            /** @type {YGenericHub} **/
            let hub = null;
            /** @type {YModule} **/
            let module = YModule.FindModuleInContext(_this34._yapi, _this34._serial + '.module');
            if (yield module.isOnline()) {
                let dev = _this34._yapi.imm_getDevice(_this34._serial);
                let baseUrl = dev.imm_getRootUrl();
                if (baseUrl.slice(-1) != '/') baseUrl = baseUrl + '/';
                hub = _this34._yapi.imm_getHub(baseUrl);
            } else {
                // test if already in bootloader
                let hubs = _this34._yapi._hubs;
                for (let i = 0; i < hubs.length; i++) {
                    let ldrs = yield hubs[i].getBootloaders();
                    if (ldrs.indexOf(_this34._serial) >= 0) {
                        hub = hubs[i];
                        break;
                    }
                }
            ...
            if (hub == null) {
                _this34.imm_progress(-1, 'Device ' + _this34._serial + ' is not detected');
                _this34._yapi._throw(YAPI.DEVICE_NOT_FOUND, 'Device ' + _this34._serial + ' is not detected', null);
                return;
            }

            try {
                yield hub.firmwareUpdate(_this34._serial, firmware, _this34._settings, function (percent, msg) {
                    _this34.imm_progress(5 + parseInt((percent * 80 + 50) / 100), msg);
                });
            } catch (e) {
                _this34.imm_progress(-1, e.message);
                _this34._yapi._throw(YAPI.IO_ERROR, e.message, null);
                return;
            }
            _this34.imm_progress(80, 'Wait for the device to restart');
            let timeout = _this34._yapi.GetTickCount() + 60000;
            yield module.clearCache();
            while (!(yield module.isOnline()) && timeout > _this34._yapi.GetTickCount()) {
                let errmsg = new YErrorMsg();
                yield _this34._yapi.Sleep(500, errmsg);
                yield _this34._yapi.UpdateDeviceList();
            }
            if (yield module.isOnline()) {
                if (_this34._settings != null) {
                    _this34.imm_progress(95, 'Restoring device settings');
                    yield module.set_allSettingsAndFiles(_this34._settings);
                    yield module.saveToFlash();
                }
                _this34.imm_progress(100, 'Success');
            } else {
                _this34.imm_progress(-1, 'Device did not reboot correctly');
            }
        })();
    }
    ...
    get_progress() {
        ...
        return _asyncToGenerator(function* () {
            if (_this35._progress >= 0) {
                yield _this35._processMore(0);
            }
            return _this35._progress;
        })();
    }
    ...
    startUpdate() {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {number} **/
            let leng;
            err = _this37._yapi.imm_bin2str(_this37._settings);
            leng = err.length;
            if (leng >= 6 && 'error:' == err.substr(0, 6)) {
                _this37._progress = -1;
                _this37._progress_msg = err.substr(6, leng - 6);
            } else {
                _this37._progress = 0;
                _this37._progress_c = 0;
                yield _this37._processMore(1);
            }
            return _this37._progress;
        })();
    }
...
class YFunction {
    ...
    get_syncProxy() {
        ...
        return _asyncToGenerator(function* () {
            if (_this38._cacheExpiration <= _this38._yapi.GetTickCount()) {
                try {
                    yield _this38.load(_this38._yapi.defaultCacheValidity);
                } catch (e) {
                    // device might be offline
                }
            }
            var res = new YFunctionProxy(_this38);
            yield res._asyncInit();
            res._module = yield (yield _this38.module()).get_syncProxy();
            return res;
        })();
    }
    ...
    get_logicalName() {
        ...
        return _asyncToGenerator(function* () {
            if (_this39._cacheExpiration <= _this39._yapi.GetTickCount()) {
                if ((yield _this39.load(_this39._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_LOGICALNAME_INVALID;
                }
            }
            return _this39._logicalName;
        })();
    }
    ...
    set_logicalName(newval) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {string} **/
            let rest_val;
            if (!_this40._yapi.CheckLogicalName(newval)) {
                return _this40._throw(_this40._yapi.INVALID_ARGUMENT, 'Invalid name :' + newval, _this40._yapi.INVALID_ARGUMENT);
            }
            rest_val = newval;
            return yield _this40._setAttr('logicalName', rest_val);
        })();
    }
    ...
    get_advertisedValue() {
        ...
        return _asyncToGenerator(function* () {
            if (_this41._cacheExpiration <= _this41._yapi.GetTickCount()) {
                if ((yield _this41.load(_this41._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_ADVERTISEDVALUE_INVALID;
                }
            }
            return _this41._advertisedValue;
        })();
    }
    ...
    set_advertisedValue(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = newval;
            return yield _this42._setAttr('advertisedValue', rest_val);
        })();
    }
    ...
    registerValueCallback(callback) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let val;
            if (callback != null) {
                yield YFunction._UpdateValueCallbackList(_this43, true);
            } else {
                yield YFunction._UpdateValueCallbackList(_this43, false);
            }
            _this43._valueCallbackFunction = callback;
            // Immediately invoke value callback with current value
            if (callback != null && (yield _this43.isOnline())) {
                val = _this43._advertisedValue;
                if (!(val == '')) {
                    yield _this43._invokeValueCallback(val);
                }
            }
            return 0;
        })();
    ...
    _invokeValueCallback(value) {
        ...
        return _asyncToGenerator(function* () {
            if (_this44._valueCallbackFunction != null) {
                yield _this44._valueCallbackFunction(_this44, value);
            } else {}
            return 0;
        })();
    }
    ...
    muteValueCallbacks() {
        ...
        return _asyncToGenerator(function* () {
            return yield _this45.set_advertisedValue('SILENT');
        })();
    }
    ...
    unmuteValueCallbacks() {
        ...
        return _asyncToGenerator(function* () {
            return yield _this46.set_advertisedValue('');
        })();
    }
     ...
     *
     * @param attrName {string} : le nom de l'attribut désiré
     *
     * @return {string} une chaîne de caractères représentant la valeur actuelle de l'attribut.
     *
    ...
    loadAttribute(attrName) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {Uint8Array} **/
            let attrVal;
            url = 'api/' + (yield _this47.get_functionId()) + '/' + attrName;
            attrVal = yield _this47._download(url);
            return _this47._yapi.imm_bin2str(attrVal);
        })();
    }
    ...
    describe() {
        ...
        return _asyncToGenerator(function* () {
            if (_this48._hwId != '') {
                return _this48._className + '(' + _this48._func + ')=' + _this48._hwId;
            }
            var resolve = _this48._yapi.imm_resolveFunction(_this48._className, _this48._func);
            if (resolve.errorType != YAPI_SUCCESS && resolve.result != _this48._func) {
                return _this48._className + '(' + _this48._func + ')=unresolved';
            }
            return _this48._className + '(' + _this48._func + ')=' + resolve.result;
        })();
    }
    ...
    get_hardwareId() {
        ...
        return _asyncToGenerator(function* () {
            if (_this49._hwId != '') {
                return _this49._hwId;
            }
            var resolve = _this49._yapi.imm_resolveFunction(_this49._className, _this49._func);
            if (resolve.errorType != YAPI_SUCCESS) {
                yield _this49.isOnline();
                resolve = _this49._yapi.imm_resolveFunction(_this49._className, _this49._func);
                if (resolve.errorType != YAPI_SUCCESS) {
                    return _this49._throw(resolve.errorType, resolve.errorMsg, Y_HARDWAREID_INVALID);
                }
            }
    ...
    get_functionId() {
        ...
        return _asyncToGenerator(function* () {
            if (_this50._funId != '') {
                return _this50._funId;
            }
            var resolve = _this50._yapi.imm_resolveFunction(_this50._className, _this50._func);
            if (resolve.errorType != YAPI_SUCCESS) {
                yield _this50.isOnline();
                resolve = _this50._yapi.imm_resolveFunction(_this50._className, _this50._func);
                if (resolve.errorType != YAPI_SUCCESS) {
                    return _this50._throw(resolve.errorType, resolve.errorMsg, Y_FUNCTIONID_INVALID);
                }
            }
    ...
    get_friendlyName() {
        ...
        return _asyncToGenerator(function* () {
            var resolve = _this51._yapi.imm_getFriendlyNameFunction(_this51._className, _this51._func);
            if (resolve.errorType != YAPI_SUCCESS) {
                yield _this51.isOnline();
                resolve = _this51._yapi.imm_getFriendlyNameFunction(_this51._className, _this51._func);
                if (resolve.errorType != YAPI_SUCCESS) {
                    return _this51._throw(resolve.errorType, resolve.errorMsg, Y_FRIENDLYNAME_INVALID);
                }
            }
            return resolve.result;
        })();
    ...
    _parse(yreq, msValidity) {
        ...
        return _asyncToGenerator(function* () {
            ...
            yreq.obj_result['_expiration'] = _this52._yapi.GetTickCount() + msValidity;
            _this52._serial = yreq.obj_result.deviceid;
            _this52._funId = yreq.obj_result.functionid;
            _this52._hwId = yreq.obj_result.hwid;
            _this52._cache = yreq.obj_result;
            // process each attribute in turn for class-oriented processing
            for (var key in yreq.obj_result) {
                _this52.imm_parseAttr(key, yreq.obj_result[key]);
            }
            yield _this52._parserHelper();
        })();
    }
    ...
    isOnline_async(func, ctx) {
        ...
        this.isOnline().then(function (res) {
            func(ctx, _this53, res);
        }).catch(function (e) {
            func(ctx, _this53, false);
        });
    }
    ...
    load_async(ms_validiy, func, ctx) {
        ...
        this.load(ms_validiy).then(function (res) {
            func(ctx, _this54, YAPI_SUCCESS);
        }).catch(function (e) {
            func(ctx, _this54, _this54.get_errorType());
        });
    }
    ...
    _getAttr(str_attr) {
        ...
        return _asyncToGenerator(function* () {
            if (_this55._cacheExpiration <= _this55._yapi.GetTickCount()) {
                ...
                if ((yield _this55.load(_this55._yapi.defaultCacheValidity)) != YAPI_SUCCESS) return null;
            }
            if (typeof _this55._cache[str_attr] == 'undefined') {
                _this55._throw(YAPI_VERSION_MISMATCH, 'No such attribute ' + str_attr + ' in function', null);
            }
            return _this55._cache[str_attr];
        })();
    }
    ...
    _getFixedAttr(str_attr) {
        ...
        return _asyncToGenerator(function* () {
            if (_this56._cacheExpiration == 0) {
                // no cached value, load from device
                if ((yield _this56.load(_this56._yapi.defaultCacheValidity)) != YAPI_SUCCESS) return null;
            }
            if (typeof _this56._cache[str_attr] == 'undefined') {
                _this56._throw(YAPI_VERSION_MISMATCH, 'No such attribute ' + str_attr + ' in function', null);
            }
            return _this56._cache[str_attr];
        })();
    }
    ...
    _setAttr(str_attr, str_newval) {
        ...
        return _asyncToGenerator(function* () {
            if (str_newval == undefined) {
                return _this57._throw(YAPI_INVALID_ARGUMENT, 'Undefined value to set for attribute ' + str_attr, null);
            }
            var attrname = encodeURIComponent(str_attr);
            var attrval = _this57.imm_escapeAttr(str_newval);
            var extra = '/' + attrname + '?' + attrname + '=' + attrval + '&.';
            if (_this57._cacheExpiration != 0) {
                _this57._cacheExpiration = _this57._yapi.GetTickCount();
                _this57._cache._expiration = _this57._cacheExpiration;
            }
            var yreq = yield _this57._yapi.funcRequest(_this57._className, _this57._func, extra);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this57._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    _download(str_path) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {YHTTPRequest} **/
            let yreq = yield _this58._yapi.devRequest(devid, 'GET /' + str_path, null, 0);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this58._throw(yreq.errorType, yreq.errorMsg, '');
            }
    ...
    _downloadOutOfBand(str_path) {
        ...
        return _asyncToGenerator(function* () {
            // get the device serial number
            /** @type {string} **/
            let devid = _this59._serial;
            if (devid == '') {
                devid = yield (yield _this59.module()).get_serialNumber();
            }
            if (devid == Y_SERIALNUMBER_INVALID) {
                return new Uint8Array(0);
            }
            /** @type {YHTTPRequest} **/
            let yreq = yield _this59._yapi.devRequest(devid, 'GET /' + str_path, null, 1);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this59._throw(yreq.errorType, yreq.errorMsg, '');
            }
            return yreq.bin_result;
        })();
    }
    ...
    _uploadWithProgress(str_path, bin_content, fun_progressCb) {
        ...
        return _asyncToGenerator(function* () {
            // get the device serial number
            var devid = _this60._serial;
            if (devid == '') {
                devid = yield (yield _this60.module()).get_serialNumber();
            }
            if (devid == Y_SERIALNUMBER_INVALID) {
                return _this60.get_errorType();
            }
            var httpreq = 'POST /upload.html';
            var len = bin_content.length;
            // convert to Uint8Array if needed
            if (typeof bin_content == 'string' || bin_content instanceof String) {
                bin_content = _this60._yapi.imm_str2bin(bin_content);
            } else if (bin_content instanceof Array) {
                bin_content = new Uint8Array(bin_content);
            }
            return yield _this60._yapi.devRequest(devid, httpreq, new YHTTPBody(str_path, bin_content, fun_progressCb), 0);
        })();
    }
    ...
    _upload(str_path, bin_content) {
        ...
        return _asyncToGenerator(function* () {
            return _this61._uploadWithProgress(str_path, bin_content, null);
        })();
    }
        ...
        if (devid == '') {
            ...
            this.module().then(function (module) {
                return module.get_serialNumber().then(function () {
                    return _this62.wait_async(callback, context);
                });
            });
            return YAPI_SUCCESS;
        }
    ...
    isOnline() {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (_this64._cacheExpiration > _this64._yapi.GetTickCount()) return true;

            // Check that the function is available without throwing exceptions
            /** @type {YFuncRequest} **/
            let yreq = yield _this64._yapi.funcRequest(_this64._className, _this64._func, '', _this64._yapi.defaultCacheValidity);
            if (yreq.errorType != YAPI_SUCCESS) {
            ...
            yield _this64._parse(yreq, _this64._yapi.defaultCacheValidity);
    ...
    load(msValidity) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YFuncRequest} **/
            let yreq = yield _this65._yapi.funcRequest(_this65._className, _this65._func, '', msValidity);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this65._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            yield _this65._parse(yreq, msValidity);
    ...
    clearCache() {
        ...
        return _asyncToGenerator(function* () {
            var devreq = yield _this66._yapi._funcDev(_this66._className, _this66._func);
            if (devreq.errorType != YAPI_SUCCESS) {
                return;
            }
            devreq.obj_result.device.imm_dropCache();
            if (_this66._cacheExpiration > 0) {
                _this66._cacheExpiration = _this66._yapi.GetTickCount();
            }
        })();
    }
    ...
    module() {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (_this67._serial != '') {
                return YModule.FindModuleInContext(_this67._yapi, _this67._serial + '.module');
            }
            var hwid = _this67._func;
            var resolve;
            if (hwid.indexOf('.') < 0) {
                resolve = _this67._yapi.imm_resolveFunction(_this67._className, _this67._func);
                if (resolve.errorType == YAPI_SUCCESS) hwid = resolve.result;
            }
            var dotidx = hwid.indexOf('.');
            if (dotidx >= 0) {
                // resolution worked
                return YModule.FindModuleInContext(_this67._yapi, hwid.substr(0, dotidx) + '.module');
            }

            // device not resolved for now, force a communication for a last chance resolution
            if ((yield _this67.load(_this67._yapi.defaultCacheValidity)) == YAPI_SUCCESS) {
                resolve = _this67._yapi.imm_resolveFunction(_this67._className, _this67._func);
            ...
            return YModule.FindModuleInContext(_this67._yapi, 'module_of_' + _this67.className + '_' + _this67._func);
        })();
    }
    ...
    get_functionDescriptor() {
        ...
        return _asyncToGenerator(function* () {
            // try to resolve the function name to a device id without query
            if (_this69._hwId != '') {
                return _this69._hwId;
            }
            var hwid = _this69._func;
            if (hwid.indexOf('.') < 0) {
                var resolve = _this69._yapi.imm_resolveFunction(_this69._className, _this69._func);
                if (resolve.errorType != YAPI_SUCCESS) hwid = resolve.result;
            }
            var dotidx = hwid.indexOf('.');
            if (dotidx >= 0) {
...
class YFunctionProxy {
    ...
    isOnline_async(func, ctx) {
        ...
        this.liveFunc.isOnline().then(function (res) {
            func(ctx, _this73, res);
        }).catch(function (e) {
            func(ctx, _this73, false);
        });
    }
    ...
    load_async(ms_validiy, func, ctx) {
        ...
        this.liveFunc.load(ms_validiy).then(function (res) {
            func(ctx, _this74, YAPI_SUCCESS);
        }).catch(function (e) {
            func(ctx, _this74, _this74.get_errorType());
        });
    }
    ...
    _download(str_path) {
        ...
        return _asyncToGenerator(function* () {
            _this75.liveFunc._download(str_path);
    ...
    _upload(str_path, bin_content) {
        ...
        return _asyncToGenerator(function* () {
            _this76.liveFunc._upload(str_path, bin_content);
            return null;
        })();
    }
    ...
    wait_async(callback, context) {
        ...
        setTimeout(function () {
            _this77.liveFunc.wait_async(function (ctx, obj) {
                try {
                    callback(ctx, _this77);
                } catch (e) {
                    console.log(e);
                }
            }, context);
...
class YModule extends YFunction {
    ...
    get_logicalName() {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YDevice} **/
            var dev = _this84.imm_getDev();
            if (dev != null && _this84._cache._expiration <= _this84._yapi.GetTickCount()) {
                return dev._logicalName;
            }
            /** @type {string|null} **/
            var json_val = yield _this84._getAttr('logicalName');
    ...
    get_syncProxy() {
        ...
        return _asyncToGenerator(function* () {
            if (_this85._cacheExpiration <= _this85._yapi.GetTickCount()) {
                try {
                    yield _this85.load(_this85._yapi.defaultCacheValidity);
                } catch (e) {
                    // device might be offline
                }
            }
            var res = new YModuleProxy(_this85);
            yield res._asyncInit();
            res._module = res;
            return res;
        })();
    ...
    get_serialNumber() {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YDevice} **/
            let dev;
            if (_this87._cacheExpiration == 0) {
                dev = _this87.imm_getDev();
                if (!(dev == null)) {
    ...
    get_productId() {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YDevice} **/
            let dev;
            if (_this88._cacheExpiration == 0) {
                dev = _this88.imm_getDev();
                if (!(dev == null)) {
    ...
    get_productRelease() {
        ...
        return _asyncToGenerator(function* () {
            ...
            return _this89._productRelease;
        })();
    }
    ...
    get_firmwareRelease() {
        ...
        return _asyncToGenerator(function* () {
            if (_this90._cacheExpiration <= _this90._yapi.GetTickCount()) {
                if ((yield _this90.load(_this90._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_FIRMWARERELEASE_INVALID;
                }
            }
            return _this90._firmwareRelease;
        })();
    }
    ...
    get_persistentSettings() {
        ...
        return _asyncToGenerator(function* () {
            ...
            return _this91._persistentSettings;
        })();
    }
    ...
    set_persistentSettings(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            return yield _this92._setAttr('persistentSettings', rest_val);
        })();
    }
    ...
    set_luminosity(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(newval);
            return yield _this94._setAttr('luminosity', rest_val);
        })();
    }
    ...
    set_beacon(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(newval);
            return yield _this96._setAttr('beacon', rest_val);
        })();
    }
    ...
    get_usbCurrent() {
        ...
        return _asyncToGenerator(function* () {
            if (_this98._cacheExpiration <= _this98._yapi.GetTickCount()) {
                if ((yield _this98.load(_this98._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_USBCURRENT_INVALID;
                }
            }
            return _this98._usbCurrent;
        })();
    }
    ...
    get_rebootCountdown() {
        ...
        return _asyncToGenerator(function* () {
            ...
            return _this99._rebootCountdown;
        })();
    }
    ...
    set_rebootCountdown(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(newval);
            return yield _this100._setAttr('rebootCountdown', rest_val);
        })();
    }
    ...
    set_userVar(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(newval);
            return yield _this102._setAttr('userVar', rest_val);
        })();
    }
    ...
    saveToFlash() {
        ...
        return _asyncToGenerator(function* () {
            return yield _this103.set_persistentSettings(Y_PERSISTENTSETTINGS_SAVED);
        })();
    }
    ...
    revertFromFlash() {
        ...
        return _asyncToGenerator(function* () {
            return yield _this104.set_persistentSettings(Y_PERSISTENTSETTINGS_LOADED);
        })();
    }
    ...
    reboot(secBeforeReboot) {
        ...
        return _asyncToGenerator(function* () {
            return yield _this105.set_rebootCountdown(secBeforeReboot);
        })();
    }
    ...
    triggerFirmwareUpdate(secBeforeReboot) {
        ...
        return _asyncToGenerator(function* () {
            return yield _this106.set_rebootCountdown(-secBeforeReboot);
        })();
    }
    ...
    checkFirmware(path, onlynew) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let tmp_res;
            if (onlynew) {
                release = _this107._yapi.imm_atoi((yield _this107.get_firmwareRelease()));
            } else {
                release = 0;
            }
            //may throw an exception
            serial = yield _this107.get_serialNumber();
            tmp_res = yield YFirmwareUpdate.CheckFirmware(serial, path, release);
            if (tmp_res.indexOf('error:') == 0) {
                _this107._throw(YAPI_INVALID_ARGUMENT, tmp_res);
            }
            return tmp_res;
        })();
    }
    ...
    updateFirmwareEx(path, force) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let settings;
            // may throw an exception
            serial = yield _this108.get_serialNumber();
            settings = yield _this108.get_allSettings();
            if (settings.length == 0) {
                _this108._throw(YAPI_IO_ERROR, 'Unable to get device settings');
                settings = _this108._yapi.imm_str2bin('error:Unable to get device settings');
            }
            return new YFirmwareUpdate(_this108._yapi, serial, path, settings, force);
        })();
    }
    ...
    updateFirmware(path) {
        ...
        return _asyncToGenerator(function* () {
            return yield _this109.updateFirmwareEx(path, false);
        })();
    }
    ...
    get_allSettings() {
        ...
        return _asyncToGenerator(function* () {
            ...
            let templist = [];
            // may throw an exception
            settings = yield _this110._download('api.json');
            if (settings.length == 0) {
                return settings;
            }
            ext_settings = ', "extras":[';
            templist = yield _this110.get_functionIds('Temperature');
            sep = '';
            for (let ii in templist) {
                if (_this110._yapi.imm_atoi((yield _this110.get_firmwareRelease())) > 9000) {
                    url = 'api/' + templist[ii] + '/sensorType';
                    t_type = _this110._yapi.imm_bin2str((yield _this110._download(url)));
                    if (t_type == 'RES_NTC') {
                        id = templist[ii].substr(11, templist[ii].length - 11);
                        temp_data_bin = yield _this110._download('extra.json?page=' + id);
                        if (temp_data_bin.length == 0) {
                            return temp_data_bin;
                        }
                        item = sep + '{"fid":"' + templist[ii] + '", "json":' + _this110._yapi.imm_bin2str(temp_data_bin) + '}\n';
            ...
            if (yield _this110.hasFunction('files')) {
                json = yield _this110._download('files.json?a=dir&f=');
                if (json.length == 0) {
                    return json;
                }
                filelist = _this110.imm_json_get_array(json);
                sep = '';
                for (let ii in filelist) {
                    name = _this110.imm_json_get_key(_this110._yapi.imm_str2bin(filelist[ii]), 'name');
                    if (name.length > 0 && !(name == 'startupConf.json')) {
                        file_data_bin = yield _this110._download(_this110.imm_escapeAttr(name));
                        file_data = _this110._yapi.imm_bin2hexstr(file_data_bin);
            ...
            res = _this110._yapi.imm_str2bin('{ "api":' + _this110._yapi.imm_bin2str(settings) + ext_settings + ']}');
            return res;
        })();
    }
    ...
    loadThermistorExtra(funcId, jsonExtra) {
        ...
        return _asyncToGenerator(function* () {
            ...
            // may throw an exception
            yield _this111._download(url);
            // add records in growing resistance value
            values = _this111.imm_json_get_array(_this111._yapi.imm_str2bin(jsonExtra));
            ofs = 0;
            size = values.length;
            while (ofs + 1 < size) {
                curr = values[ofs];
    ...
    set_extraSettings(jsonExtra) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {string} **/
            let data;
            extras = _this112.imm_json_get_array(_this112._yapi.imm_str2bin(jsonExtra));
            for (let ii in extras) {
                functionId = _this112.imm_get_json_path(extras[ii], 'fid');
                functionId = _this112.imm_decode_json_string(functionId);
                data = _this112.imm_get_json_path(extras[ii], 'json');
                if (yield _this112.hasFunction(functionId)) {
                    yield _this112.loadThermistorExtra(functionId, data);
                }
            }
            return YAPI_SUCCESS;
        })();
    ...
    set_allSettingsAndFiles(settings) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {string} **/
            let json_extra;
            json = _this113._yapi.imm_bin2str(settings);
            json_api = _this113.imm_get_json_path(json, 'api');
            if (json_api == '') {
                return yield _this113.set_allSettings(settings);
            }
            json_extra = _this113.imm_get_json_path(json, 'extras');
            if (!(json_extra == '')) {
                yield _this113.set_extraSettings(json_extra);
            }
            yield _this113.set_allSettings(_this113._yapi.imm_str2bin(json_api));
            if (yield _this113.hasFunction('files')) {
                /** @type {string[]} **/
                let files = [];
                /** @type {string} **/
                let res;
                ...
                /** @type {string} **/
                let data;
                down = yield _this113._download('files.json?a=format');
                res = _this113.imm_get_json_path(_this113._yapi.imm_bin2str(down), 'res');
                res = _this113.imm_decode_json_string(res);
                if (!(res == 'ok')) {
                    return _this113._throw(YAPI_IO_ERROR, 'format failed', YAPI_IO_ERROR);
                }
                json_files = _this113.imm_get_json_path(json, 'files');
                files = _this113.imm_json_get_array(_this113._yapi.imm_str2bin(json_files));
                for (let ii in files) {
                    name = _this113.imm_get_json_path(files[ii], 'name');
                    name = _this113.imm_decode_json_string(name);
                    data = _this113.imm_get_json_path(files[ii], 'data');
                    data = _this113.imm_decode_json_string(data);
                    yield _this113._upload(name, _this113._yapi.imm_hexstr2bin(data));
                }
            }
            return YAPI_SUCCESS;
        })();
    ...
    hasFunction(funcId) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let fid;
            // may throw an exception
            count = yield _this114.functionCount();
            i = 0;
            while (i < count) {
                fid = yield _this114.functionId(i);
                if (fid == funcId) {
                    return true;
                }
                i = i + 1;
    ...
    get_functionIds(funType) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let res = [];
            // may throw an exception
            count = yield _this115.functionCount();
            i = 0;
            while (i < count) {
                ftype = yield _this115.functionType(i);
                if (ftype == funType) {
                    res.push((yield _this115.functionId(i)));
                } else {
                    ftype = yield _this115.functionBaseType(i);
                    if (ftype == funType) {
                        res.push((yield _this115.functionId(i)));
                    }
                }
    ...
    calibScale(unit_name, sensorType) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (unit_name == 'C') {
                ...
                if (_this116._yapi.imm_atoi(sensorType) < 8) {
                    return 16;
                } else {
                    return 100;
                }
    ...
    calibConvert(param, currentFuncValue, unit_name, sensorType) {
        ...
        return _asyncToGenerator(function* () {
            ...
            paramVer = yield _this117.calibVersion(param);
            funVer = yield _this117.calibVersion(currentFuncValue);
            funScale = yield _this117.calibScale(unit_name, sensorType);
            funOffset = yield _this117.calibOffset(unit_name);
            paramScale = funScale;
            paramOffset = funOffset;
            if (funVer < 3) {
                if (funVer == 2) {
                    words = _this117._yapi.imm_decodeWords(currentFuncValue);
                    ...
                    if (funVer == 1) {
                        if (currentFuncValue == '' || _this117._yapi.imm_atoi(currentFuncValue) > 10) {
                            funScale = 0;
                        }
                    }
                }
            ...
            if (paramVer < 3) {
                if (paramVer == 2) {
                    words = _this117._yapi.imm_decodeWords(param);
                    if (words[0] == 1366 && words[1] == 12500) {
                        paramScale = 1;
                        paramOffset = 0;
                    } else {
                    ...
                    if (paramVer == 1) {
                        ...
                        for (let ii in words_str) {
                            words.push(_this117._yapi.imm_atoi(words_str[ii]));
                        }
                        if (param == '' || words[0] > 10) {
                            paramScale = 0;
                        }
                ...
                while (i < calibData.length) {
                    if (paramScale > 0) {
                        ...
                        calibData[i] = _this117._yapi.imm_decimalToDouble(Math.round(calibData[i]));
                    }
                    i = i + 1;
                }
            } else {
                iCalib = _this117._yapi.imm_decodeFloats(param);
            ...
            if (funVer >= 3) {
                ...
                if (funVer >= 1) {
                    ...
                    while (i < 2 * nPoints) {
                        if (funScale == 0) {
                            wordVal = _this117._yapi.imm_doubleToDecimal(Math.round(calibData[i]));
                        } else {
    ...
    set_allSettings(settings) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {boolean} **/
            let found;
            tmp = _this118._yapi.imm_bin2str(settings);
            tmp = _this118.imm_get_json_path(tmp, 'api');
            if (!(tmp == '')) {
                settings = _this118._yapi.imm_str2bin(tmp);
            }
            oldval = '';
            newval = '';
            old_json_flat = _this118.imm_flattenJsonStruct(settings);
            old_dslist = _this118.imm_json_get_array(old_json_flat);
            for (let ii in old_dslist) {
                each_str = _this118.imm_json_get_string(_this118._yapi.imm_str2bin(old_dslist[ii]));
                leng = each_str.length;
                eqpos = each_str.indexOf('=');
                if (eqpos < 0 || leng == 0) {
                    _this118._throw(YAPI_INVALID_ARGUMENT, 'Invalid settings');
            ...
            // may throw an exception
            actualSettings = yield _this118._download('api.json');
            actualSettings = _this118.imm_flattenJsonStruct(actualSettings);
            new_dslist = _this118.imm_json_get_array(actualSettings);
            for (let ii in new_dslist) {
                each_str = _this118.imm_json_get_string(_this118._yapi.imm_str2bin(new_dslist[ii]));
                leng = each_str.length;
                eqpos = each_str.indexOf('=');
                if (eqpos < 0 || leng == 0) {
                    _this118._throw(YAPI_INVALID_ARGUMENT, 'Invalid settings');
            ...
            while (i < new_jpath.length) {
                ...
                if (do_update) {
                    if (attr == 'calibrationParam') {
                        ...
                        newval = yield _this118.calibConvert(old_calib, new_val_arr[i], unit_name, sensorType);
                        url = 'api/' + fun + '.json?' + attr + '=' + _this118.imm_escapeAttr(newval);
                        yield _this118._download(url);
                    } else {
                        url = 'api/' + fun + '.json?' + attr + '=' + _this118.imm_escapeAttr(oldval);
                        if (attr == 'resolution') {
            ...
            for (let ii in restoreLast) {
                yield _this118._download(restoreLast[ii]);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    download(pathname) {
        ...
        return _asyncToGenerator(function* () {
            return yield _this119._download(pathname);
        })();
    }
    ...
    get_icon2d() {
        ...
        return _asyncToGenerator(function* () {
            return yield _this120._download('icon2d.png');
        })();
    }
    ...
    get_lastLogs() {
        ...
        return _asyncToGenerator(function* () {
            ...
            let content;
            // may throw an exception
            content = yield _this121._download('logs.txt');
            return _this121._yapi.imm_bin2str(content);
        })();
    }
    ...
    log(text) {
        ...
        return _asyncToGenerator(function* () {
            return yield _this122._upload('logs.txt', _this122._yapi.imm_str2bin(text));
        })();
    }
...
class YSensor extends YFunction {
    ...
    get_syncProxy() {
        ...
        return _asyncToGenerator(function* () {
            if (_this123._cacheExpiration <= _this123._yapi.GetTickCount()) {
                try {
                    yield _this123.load(_this123._yapi.defaultCacheValidity);
                } catch (e) {
                    // device might be offline
                }
            }
            var res = new YSensorProxy(_this123);
            yield res._asyncInit();
            res._module = yield (yield _this123.module()).get_syncProxy();
            return res;
        })();
    }
    ...
    get_unit() {
        ...
        return _asyncToGenerator(function* () {
            if (_this124._cacheExpiration <= _this124._yapi.GetTickCount()) {
                if ((yield _this124.load(_this124._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_UNIT_INVALID;
                }
            }
            return _this124._unit;
        })();
    }
    ...
    get_currentValue() {
        ...
        return _asyncToGenerator(function* () {
            ...
            res = yield _this125._applyCalibration(_this125._currentRawValue);
            if (res == Y_CURRENTVALUE_INVALID) {
                res = _this125._currentValue;
            }
            res = res * _this125._iresol;
    ...
    set_lowestValue(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            return yield _this126._setAttr('lowestValue', rest_val);
        })();
    }
    ...
    get_lowestValue() {
        ...
        return _asyncToGenerator(function* () {
            ...
            res = _this127._lowestValue * _this127._iresol;
    ...
    set_highestValue(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(Math.round(newval * 65536.0));
            return yield _this128._setAttr('highestValue', rest_val);
        })();
    }
    ...
    get_highestValue() {
        ...
        return _asyncToGenerator(function* () {
            ...
            res = _this129._highestValue * _this129._iresol;
            return Math.round(res) / _this129._iresol;
        })();
    }
    ...
    get_currentRawValue() {
        ...
        return _asyncToGenerator(function* () {
            if (_this130._cacheExpiration <= _this130._yapi.GetTickCount()) {
                if ((yield _this130.load(_this130._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_CURRENTRAWVALUE_INVALID;
                }
            }
            return _this130._currentRawValue;
        })();
    }
    ...
    set_logFrequency(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = newval;
            return yield _this132._setAttr('logFrequency', rest_val);
        })();
    }
    ...
    get_reportFrequency() {
        ...
        return _asyncToGenerator(function* () {
            ...
            return _this133._reportFrequency;
        })();
    }
    ...
    set_reportFrequency(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = newval;
            return yield _this134._setAttr('reportFrequency', rest_val);
        })();
    }
    ...
    get_calibrationParam() {
        ...
        return _asyncToGenerator(function* () {
            if (_this135._cacheExpiration <= _this135._yapi.GetTickCount()) {
                if ((yield _this135.load(_this135._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_CALIBRATIONPARAM_INVALID;
                }
            }
            return _this135._calibrationParam;
        })();
    }
    ...
    set_calibrationParam(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = newval;
            return yield _this136._setAttr('calibrationParam', rest_val);
        })();
    }
    ...
    set_resolution(newval) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            rest_val = String(Math.round(newval * 65536.0));
            return yield _this137._setAttr('resolution', rest_val);
        })();
    }
    ...
    get_resolution() {
        ...
        return _asyncToGenerator(function* () {
            if (_this138._cacheExpiration <= _this138._yapi.GetTickCount()) {
                if ((yield _this138.load(_this138._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_RESOLUTION_INVALID;
                }
            }
            return _this138._resolution;
        })();
    }
    ...
    get_sensorState() {
        ...
        return _asyncToGenerator(function* () {
            if (_this139._cacheExpiration <= _this139._yapi.GetTickCount()) {
                if ((yield _this139.load(_this139._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return Y_SENSORSTATE_INVALID;
                }
            }
            return _this139._sensorState;
        })();
    }
    ...
    _parserHelper() {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this140._caltyp = -1;
            _this140._scale = -1;
            _this140._isScal32 = false;
            _this140._calpar.length = 0;
            _this140._calraw.length = 0;
            _this140._calref.length = 0;
            // Store inverted resolution, to provide better rounding
            if (_this140._resolution > 0) {
                _this140._iresol = Math.round(1.0 / _this140._resolution);
            } else {
                _this140._iresol = 10000;
                _this140._resolution = 0.0001;
            }
            // Old format: supported when there is no calibration
            if (_this140._calibrationParam == '' || _this140._calibrationParam == '0') {
                _this140._caltyp = 0;
                return 0;
            }
            if (_this140._calibrationParam.indexOf(',') >= 0) {
                iCalib = _this140._yapi.imm_decodeFloats(_this140._calibrationParam);
                _this140._caltyp = parseInt(iCalib[0] / 1000, 10);
                if (_this140._caltyp > 0) {
                    if (_this140._caltyp < YOCTO_CALIB_TYPE_OFS) {
                        _this140._caltyp = -1;
                        return 0;
                    }
                    _this140.imm_calhdl = _this140._yapi.imm_getCalibrationHandler(_this140._caltyp);
                    if (!(_this140.imm_calhdl != null)) {
                        _this140._caltyp = -1;
                        return 0;
                    }
                }
                _this140._isScal = true;
                _this140._isScal32 = true;
                _this140._offset = 0;
                _this140._scale = 1000;
                maxpos = iCalib.length;
                _this140._calpar.length = 0;
                position = 1;
                while (position < maxpos) {
                    _this140._calpar.push(iCalib[position]);
                    position = position + 1;
                }
                _this140._calraw.length = 0;
                _this140._calref.length = 0;
                ...
                while (position + 1 < maxpos) {
                    ...
                    fRef = fRef / 1000.0;
                    _this140._calraw.push(fRaw);
                    _this140._calref.push(fRef);
                    position = position + 2;
                }
            } else {
                iCalib = _this140._yapi.imm_decodeWords(_this140._calibrationParam);
                if (iCalib.length < 2) {
                    _this140._caltyp = -1;
                    return 0;
                }
                _this140._isScal = iCalib[1] > 0;
                if (_this140._isScal) {
                    _this140._offset = iCalib[0];
                    if (_this140._offset > 32767) {
                        _this140._offset = _this140._offset - 65536;
                    }
                    _this140._scale = iCalib[1];
                    _this140._decexp = 0;
                } else {
                    _this140._offset = 0;
                    _this140._scale = 1;
                    _this140._decexp = 1.0;
                    position = iCalib[0];
                    while (position > 0) {
                        _this140._decexp = _this140._decexp * 10;
                        position = position - 1;
                    }
                }
                if (iCalib.length == 2) {
                ...
                _this140._caltyp = iCalib[2];
                _this140.imm_calhdl = _this140._yapi.imm_getCalibrationHandler(_this140._caltyp);
                if (_this140._caltyp <= 10) {
                    maxpos = _this140._caltyp;
                } else {
                    if (_this140._caltyp <= 20) {
                        maxpos = _this140._caltyp - 10;
                    } else {
                        maxpos = 5;
                    }
                }
                ...
                while (position + 1 < maxpos) {
                    ...
                    _this140._calpar.push(iRaw);
                    _this140._calpar.push(iRef);
                    if (_this140._isScal) {
                        fRaw = iRaw;
                        fRaw = (fRaw - _this140._offset) / _this140._scale;
                        fRef = iRef;
                        fRef = (fRef - _this140._offset) / _this140._scale;
                        _this140._calraw.push(fRaw);
                        _this140._calref.push(fRef);
                    } else {
                        _this140._calraw.push(_this140._yapi.imm_decimalToDouble(iRaw));
                        _this140._calref.push(_this140._yapi.imm_decimalToDouble(iRef));
                    }
                    position = position + 2;
                }
            }
    ...
    isSensorReady() {
        ...
        return _asyncToGenerator(function* () {
            if (!(yield _this141.isOnline())) {
                return false;
            }
            if (!(_this141._sensorState == 0)) {
                return false;
            }
            return true;
        })();
    ...
    startDataLogger() {
        ...
        return _asyncToGenerator(function* () {
            ...
            let res;
            // may throw an exception
            res = yield _this142._download('api/dataLogger/recording?recording=1');
            if (!(res.length > 0)) {
                return _this142._throw(YAPI_IO_ERROR, 'unable to start datalogger', YAPI_IO_ERROR);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    stopDataLogger() {
        ...
        return _asyncToGenerator(function* () {
            ...
            let res;
            // may throw an exception
            res = yield _this143._download('api/dataLogger/recording?recording=0');
            if (!(res.length > 0)) {
                return _this143._throw(YAPI_IO_ERROR, 'unable to stop datalogger', YAPI_IO_ERROR);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    get_recordedData(startTime, endTime) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let funit;
            // may throw an exception
            funcid = yield _this144.get_functionId();
            funit = yield _this144.get_unit();
            return new YDataSet(_this144, funcid, funit, startTime, endTime);
        })();
    }
    ...
    registerTimedReportCallback(callback) {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this145._timedReportCallbackSensor = callback;
            return 0;
        })();
    }
    ...
    _invokeTimedReportCallback(value) {
        ...
        return _asyncToGenerator(function* () {
            if (_this146._timedReportCallbackSensor != null) {
                yield _this146._timedReportCallbackSensor(_this146, value);
            } else {}
            return 0;
        })();
    }
    ...
    calibrateFromPoints(rawValues, refValues) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let rest_val;
            // may throw an exception
            rest_val = yield _this147._encodeCalibrationPoints(rawValues, refValues);
            return yield _this147._setAttr('calibrationParam', rest_val);
        })();
    }
    ...
    loadCalibrationPoints(rawValues, refValues) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (_this148._scale == 0) {
                if ((yield _this148.load(_this148._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return YAPI_DEVICE_NOT_FOUND;
                }
            }
            if (_this148._caltyp < 0) {
                _this148._throw(YAPI_NOT_SUPPORTED, 'Calibration parameters format mismatch. Please upgrade your library or firmware.');
                return YAPI_NOT_SUPPORTED;
            }
            rawValues.length = 0;
            refValues.length = 0;
            for (let ii in _this148._calraw) {
                rawValues.push(_this148._calraw[ii]);
            }
            for (let ii in _this148._calref) {
                refValues.push(_this148._calref[ii]);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    _encodeCalibrationPoints(rawValues, refValues) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (npt != refValues.length) {
                _this149._throw(YAPI_INVALID_ARGUMENT, 'Invalid calibration parameters (size mismatch)');
            ...
            // Load function parameters if not yet loaded
            if (_this149._scale == 0) {
                if ((yield _this149.load(_this149._yapi.defaultCacheValidity)) != YAPI_SUCCESS) {
                    return YAPI_INVALID_STRING;
                }
            }
            // Detect old firmware
            if (_this149._caltyp < 0 || _this149._scale < 0) {
                _this149._throw(YAPI_NOT_SUPPORTED, 'Calibration parameters format mismatch. Please upgrade your library or firmware.');
                return '0';
            }
            if (_this149._isScal32) {
                ...
                if (_this149._isScal) {
                    ...
                    idx = 0;
                    while (idx < npt) {
                        iRaw = Math.round(rawValues[idx] * _this149._scale + _this149._offset);
                        iRef = Math.round(refValues[idx] * _this149._scale + _this149._offset);
                    ...
                    idx = 0;
                    while (idx < npt) {
                        iRaw = _this149._yapi.imm_doubleToDecimal(rawValues[idx]);
                        iRef = _this149._yapi.imm_doubleToDecimal(refValues[idx]);
                        res = res + ',' + String(Math.round(iRaw)) + ',' + String(Math.round(iRef));
                        idx = idx + 1;
                    }
                }
    ...
    _applyCalibration(rawValue) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (_this150._caltyp == 0) {
                return rawValue;
            }
            if (_this150._caltyp < 0) {
                return Y_CURRENTVALUE_INVALID;
            }
            if (!(_this150.imm_calhdl != null)) {
                return Y_CURRENTVALUE_INVALID;
            }
            return _this150.imm_calhdl(rawValue, _this150._caltyp, _this150._calpar, _this150._calraw, _this150._calref);
        })();
    }
    ...
    _decodeTimedReport(timestamp, report) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {number} **/
            let maxVal;
            startTime = _this151._prevTimedReport;
            endTime = timestamp;
            _this151._prevTimedReport = endTime;
            if (startTime == 0) {
                startTime = endTime;
            }
            if (report[0] == 2) {
                if (report.length <= 5) {
                    ...
                    if (_this151._caltyp != 0) {
                        if (_this151.imm_calhdl != null) {
                            avgVal = _this151.imm_calhdl(avgVal, _this151._caltyp, _this151._calpar, _this151._calraw, _this151._calref);
                        }
                    }
                    minVal = avgVal;
                    maxVal = avgVal;
                    ...
                    if (_this151._caltyp != 0) {
                        if (_this151.imm_calhdl != null) {
                            avgVal = _this151.imm_calhdl(avgVal, _this151._caltyp, _this151._calpar, _this151._calraw, _this151._calref);
                            minVal = _this151.imm_calhdl(minVal, _this151._caltyp, _this151._calpar, _this151._calraw, _this151._calref);
                            maxVal = _this151.imm_calhdl(maxVal, _this151._caltyp, _this151._calpar, _this151._calraw, _this151._calref);
                        }
                    }
                }
            } else {
                if (report[0] == 0) {
                    ...
                    if (_this151._isScal) {
                        avgVal = _this151.imm_decodeVal(avgRaw);
                    } else {
                        if ((byteVal & 0x80) != 0) {
                            avgRaw = avgRaw - poww;
                        }
                        avgVal = _this151.imm_decodeAvg(avgRaw);
                    }
                    minVal = avgVal;
                    maxVal = avgVal;
                } else {
                    ...
                    minVal = _this151.imm_decodeVal(minRaw);
                    avgVal = _this151.imm_decodeAvg(avgRaw);
                    maxVal = _this151.imm_decodeVal(maxRaw);
                }
            }
...
class YGenericHub {
    ...
    testHub(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            ...
            /** @type {YHTTPRequest} **/
            let yreq = yield _this152.request('GET', '/api/module.json', null, 0);
            if (yreq.errorType != YAPI_SUCCESS) {
    ...
    imm_testHubAgainLater() {
        ...
        this._reconnectionTimer = setTimeout(function () {
            _this153._reconnectionTimer = null;
            _this153.testHub(new YErrorMsg());
        }, this.retryDelay);
    }
    ...
    hubUpdateDeviceList() {
        ...
        return _asyncToGenerator(function* () {
            ...
            try {
                /** @type {YDevice} **/
                let hubDev = _this154._yapi.imm_getDevice(_this154.urlInfo.url);
                hubDev.imm_dropCache();
                /** @type {number} **/
                let retcode = yield hubDev.refresh();
                if (retcode != YAPI_SUCCESS) {
                    return _this154._throw(retcode, hubDev._lastErrorMsg, retcode);
                }
                /** @type {YHTTPRequest} **/
                let yreq = yield hubDev.requestAPI(_this154._yapi.defaultCacheValidity);
                if (yreq.errorType != YAPI_SUCCESS) {
                    return yreq;
                }
                let loadval = null;
                try {
                    loadval = JSON.parse(_this154._yapi.imm_bin2str(yreq.bin_result));
                } catch (err) {}
                if (!loadval) {
                    return _this154._throw(YAPI_IO_ERROR, 'Request failed, could not parse API result for ' + hubDev.imm_describe(), YAPI_IO_ERROR);
                }
                let whitePages = loadval['services']['whitePages'];
                let yellowPages = loadval['services']['yellowPages'];
                if (whitePages == undefined) {
                    return _this154._throw(YAPI_IO_ERROR, 'Device ' + hubDev.imm_describe() + ' is not a hub', YAPI_IO_ERROR);
                }
                retcode = yield _this154._yapi.updateDeviceList_process(_this154, hubDev, whitePages, yellowPages);
                if (retcode != YAPI_SUCCESS) {
                    return _this154._throw(_this154._yapi._lastErrorType, _this154._yapi._lastErrorMsg, _this154._yapi._lastErrorType);
                }

                // reset device list cache timeout for this hub
                _this154.devListExpires = _this154._yapi.GetTickCount() + _this154.devListValidity;
                return YAPI_SUCCESS;
            } catch (e) {
    ...
    getBootloaders() {
        ...
        return _asyncToGenerator(function* () {
            let yreq = yield _this155.request('GET', '/flash.json?a=list', null, 1);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this155._throw(yreq.errorType, yreq.errorMsg, []);
            }
    ...
    firmwareUpdate(serial, firmware, settings, progress) {
        ...
        return _asyncToGenerator(function* () {
            ...
            let _throw = function (msg) {
                return _this156._throw(YAPI.IO_ERROR, msg, [msg]);
            };

            progress(5, 'Check bootloader type');
            // Get the hub own serial number
            let yreq = yield _this156.request('GET', '/api/module.json', null, 0);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _throw('Hub is not responding');
            }
            let json = JSON.parse(_this156._yapi.imm_bin2str(yreq.bin_result));
            ...
            if (ownSerial.slice(0, 7) == 'VIRTHUB') {
                ...
                yreq = yield _this156.request('GET', '/bySerial/' + serial + '/flash.json?a=state', null, 0);
                if (yreq.errorType == YAPI_SUCCESS) {
                    use_self_flash = true;
                    baseUrl = '/bySerial/' + serial;
                }
            }
            let bootloaders = yield _this156.getBootloaders();
            ...
            yreq = yield _this156.request('GET', baseUrl + '/flash.json?a=state', null, 0);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _throw('Cannot check state of firmware upload');
            }
            json = JSON.parse(_this156._yapi.imm_bin2str(yreq.bin_result));
            ...
            yreq = yield _this156.request('POST', baseUrl + '/upload.html', new YHTTPBody('firmware', firmware.imm_getData(), progressCb), 0);
            if (yreq.errorType != YAPI_SUCCESS) {
            ...
            if (use_self_flash) {
                let settingsStr = _this156._yapi.imm_bin2str(settings);
                ...
                let startupConf = _this156._yapi.imm_str2bin(JSON.stringify(startupApi));
                progress(38, 'Save current settings');
                yreq = yield _this156.request('POST', baseUrl + '/upload.html', new YHTTPBody('startupConf.json', startupConf, null), 0);
                if (yreq.errorType != YAPI_SUCCESS) {
                    return _throw('Failed to save settings on hub');
                }
                progress(39, 'Save current settings');
                yreq = yield _this156.request('POST', baseUrl + '/upload.html', new YHTTPBody('firmwareConf', startupConf, null), 0);
                if (yreq.errorType != YAPI_SUCCESS) {
                    return _throw('Failed to save settings on hub');
                }
            }
            ...
            if (use_self_flash) {
                ...
                yield _this156.request('GET', baseUrl + '/api/module/rebootCountdown?rebootCountdown=-1003', null, 0);
                yield _this156._yapi.Sleep(7000);
            } else {
                // reboot device to bootloader if needed
                if (need_reboot) {
                    // reboot subdevice
                    yield _this156.request('GET', '/bySerial/' + serial + '/api/module/rebootCountdown?rebootCountdown=-2', null, 0);
                }
                ...
                do {
                    bootloaders = yield _this156.getBootloaders();
                    for (i = 0; i < bootloaders.length; i++) {
                        let bl = bootloaders[i];
                        if (bl == serial) {
                            found = true;
                    ...
                    if (!found) {
                        yield _this156._yapi.Sleep(500);
                    }
                ...
                let checkFlash = function () {
                    _this156.request('GET', baseUrl + '/flash.json?a=state', null, 1).then(function (flashReq) {
                        ...
                        console.log(e);checkTimer = setTimeout(checkFlash, 500);
                    });
                };
                checkTimer = setTimeout(checkFlash, 1000);
                yreq = yield _this156.request('GET', '/flash.json?a=flash&s=' + serial, null, 0);
                clearTimeout(checkTimer);
                if (yreq.errorType != YAPI_SUCCESS) {
                    return _throw('Cannot check state of firmware flash');
                }
                return JSON.parse(_this156._yapi.imm_bin2str(yreq.bin_result));
            }
            return null;
        })();
    }
...
class YHttpHub extends YGenericHub {
    ...
    testHub(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            if (_this158.disconnecting) {
                return YAPI_IO_ERROR;
            }
            let args = '?len=' + _this158.notiflen.toString();
            if (_this158.notifPos > 0) {
                args += '&abs=' + _this158.notifPos.toString();
            }
            _this158.notbynOpenPromise = new Promise(function (resolve, reject) {
                _this158.notbynRequest = new XMLHttpRequest();
                _this158.notbynRequest.open('GET', _this158.urlInfo.url + 'not.byn' + args, true, '', '');
                _this158.notbynRequest.overrideMimeType('text/plain; charset=x-user-defined');
                _this158.notbynRequest.onreadystatechange = function () {
                    if (_this158.disconnecting) {
                        return;
                    }
                    if (_this158.notbynRequest.readyState >= 3) {
                        resolve(YAPI_SUCCESS);
                        if (_this158.notbynRequest.readyState == 4 && parseInt(_this158.notbynRequest.status) != 200 && parseInt(_this158.notbynRequest.status) != 304) {
                            // connection error
                            _this158.imm_testHubAgainLater();
                        } else {
                            // receiving data properly
                            if (_this158.notbynRequest.readyState == 3) {
                                // when using reconnection mode, ignore state 3
                                if (_this158.notiflen == 1) return;
                            }
                            let newlen = _this158.notbynRequest.responseText.length;
                            if (newlen > _this158.currPos) {
                                _this158._yapi.parseEvents(_this158, _this158.notbynRequest.responseText.substr(_this158.currPos, newlen - _this158.currPos));
                            }
                            // trigger immediately a new connection if closed in success
                            if (_this158.notbynRequest.readyState == 4 && parseInt(_this158.notbynRequest.status) != 0) {
                                _this158.currPos = 0;
                                _this158.testHub(new YErrorMsg());
                            }
                        }
                    }
                };
                _this158.notbynRequest.send('');
            });
            return yield _this158.notbynOpenPromise;
        })();
    }
    ...
    request(method, devUrl, obj_body, tcpchan) {
        ...
        return _asyncToGenerator(function* () {
            let httpPromise = new Promise(function (resolve, reject) {
                let prefix = _this159.urlInfo.url.slice(0, -1);
                let httpRequest = new XMLHttpRequest();
                httpRequest.open(method, prefix + devUrl, true, _this159.urlInfo.user, _this159.urlInfo.pass);
                httpRequest.onreadystatechange = function () {
                    if (httpRequest.readyState == 4) {
                        ...
                        if (httpRequest.status != 200 && httpRequest.status != 304) {
                            ...
                            yreq.bin_result = _this159._yapi.imm_str2bin(httpRequest.responseText);
                        }
                        resolve(yreq);
                    }
                };
    ...
    disconnect() {
        ...
        return _asyncToGenerator(function* () {
            _this160.imm_commonDisconnect();
            _this160.notbynRequest.abort();
        })();
    }
}
...
class YHttpNodeHub extends YGenericHub {
    ...
    testHub(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            if (_this161.disconnecting) {
                return YAPI_IO_ERROR;
            }
            let args = '';
            if (_this161.notifPos > 0) {
                args = '?abs=' + _this161.notifPos.toString();
            }
            let options = {
                method: 'GET',
                hostname: _this161.urlInfo.host,
                port: _this161.urlInfo.port,
                path: '/not.byn' + args
            };
            _this161.notbynOpenPromise = new Promise(function (resolve, reject) {
                _this161.notbynRequest = _this161.http.request(options, function (res) {
                    if (_this161.disconnecting) {
                        return;
                    }
                    if (res.statusCode != 200 && res.statusCode != 304) {
                        // connection error
                        _this161.imm_testHubAgainLater();
                    } else {
                        resolve(YAPI_SUCCESS);
                        res.on('data', function (chunk) {
                            // receiving data properly
                            _this161._yapi.parseEvents(_this161, _this161._yapi.imm_bin2str(chunk));
                        });
                        res.on('end', function () {
                            // trigger immediately a new connection if closed in success
                            _this161.currPos = 0;
                            _this161.testHub(new YErrorMsg());
                        });
                    }
                });
                _this161.notbynRequest.on('error', function () {
                    // connection aborted, need to reconnect ASAP
                    _this161.imm_testHubAgainLater();
                });
                _this161.notbynRequest.end();
            });
            return yield _this161.notbynOpenPromise;
        })();
    }
    ...
    request(str_method, devUrl, obj_body, tcpchan) {
        ...
        return _asyncToGenerator(function* () {
            let options = {
                method: str_method,
                hostname: _this162.urlInfo.host,
                port: _this162.urlInfo.port,
                path: devUrl
            };
            if (obj_body) {
                options.boundary = _this162.imm_getBoundary();
                options.headers = {
            ...
            let httpPromise = new Promise(function (resolve, reject) {
                let response = new Buffer(0);
                let httpRequest = _this162.http.request(options, function (res) {
                    if (res.statusCode != 200 && res.statusCode != 304) {
                        // connection error
                        let yreq = new YHTTPRequest(null);
                        yreq.errorType = httpRequest.status == 401 ? YAPI_UNAUTHORIZED : YAPI_NOT_SUPPORTED;
                        yreq.errorMsg = 'HTTP Error ' + res.statusCode.toString() + ' on ' + _this162.urlInfo.url.slice(0, -1) + devUrl;
                        resolve(yreq);
                    } else {
                        res.on('data', function (chunk) {
                            // receiving data properly
                ...
                if (obj_body) {
                    httpRequest.write(new Buffer(_this162.imm_formEncodeBody(obj_body, options.boundary)));
                }
                httpRequest.end();
            });
            return httpPromise;
    ...
    disconnect() {
        ...
        return _asyncToGenerator(function* () {
            _this163.imm_commonDisconnect();
            _this163.notbynRequest.abort();
        })();
    }
}
...
class YHttpCallbackHub extends YGenericHub {
    ...
    testHub(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            yield _this164.httpCallbackPromise;
            if (_this164._incomingMessage.method != 'POST') {
                errmsg.msg = 'HTTP POST expected';
                return YAPI_INVALID_ARGUMENT;
            }
            if (_this164._callbackData.length == 0) {
                errmsg.msg = 'Empty POST body';
                return YAPI_NO_MORE_DATA;
            }
            if (_this164.urlInfo.pass != '') {
                // callback data signed, verify signature
                if (!_this164._callbackCache.sign) {
                    errmsg.msg = 'missing signature from incoming YoctoHub (callback password required)';
                    _this164._callbackCache = [];
                    return YAPI_NO_MORE_DATA;
                }
                let sign = _this164._callbackCache['sign'];
                let pass = _this164.urlInfo.pass;
                let salt;
                if (pass.length == 32) {
                    salt = pass.toLowerCase();
                } else {
                    salt = _this164._yapi.imm_bin2hexstr(_this164._yapi.imm_yMD5(pass)).toLowerCase();
                }
                let patched = _this164._yapi.imm_bin2str(_this164._callbackData).replace(sign, salt);
                let check = _this164._yapi.imm_bin2hexstr(_this164._yapi.imm_yMD5(patched));
    ...
    request(str_method, devUrl, obj_body, tcpchan) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (str_method == 'POST' && obj_body) {
                let boundary = _this165.imm_getBoundary();
                let body = _this165.imm_formEncodeBody(obj_body, boundary);
                _this165._serverResponse.write('\n@YoctoAPI:' + str_method + ' ' + devUrl + ' ' + body.length + ':' + boundary + '\n');
                _this165._serverResponse.write(new Buffer(body));
                ...
                if (devUrl.indexOf('?') == -1 || devUrl.indexOf('/logs.txt') != -1 || devUrl.indexOf('/logger.json') != -1 || devUrl.indexOf('/ping.txt') != -1 || devUrl.indexOf('/files.json?a=dir') != -1) {
                    ...
                    if (!_this165._callbackCache[devUrl]) {
                        ...
                        var jsonres = _this165._callbackCache[devUrl];
                        if (subfun) {
                            jsonres = jsonres[subfun[1]];
                        }
                        yreq.bin_result = _this165._yapi.imm_str2bin(JSON.stringify(jsonres));
                    }
                } else {
...
class YWebSocketHub extends YGenericHub {
    ...
    testHub(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            if (_this166.disconnecting) {
                return YAPI_IO_ERROR;
            }
            // Open WebSocket connection
            _this166._connectionState = _this166._WS_CONNECTING;
            _this166.imm_webSocketOpen(_this166.urlInfo.url + 'not.byn');
            _this166.websocketOpenPromise = new Promise(function (resolve, reject) {
                if (_this166.disconnecting) {
                    resolve(YAPI_IO_ERROR);
                } else {
                    _this166.websocket.onmessage = function (evt) {
                        _this166._webSocketMsg(new Uint8Array(evt.data));
                        if (_this166._connectionState == _this166._WS_CONNECTED) {
                            resolve(YAPI_SUCCESS);
                        } else if (_this166._connectionState == _this166._WS_DEAD) {
                            errmsg.msg = _this166._session_error;
                            console.log('WebSocket error: ' + _this166._session_error);
                            resolve(_this166._session_errno == 401 ? YAPI_UNAUTHORIZED : YAPI_IO_ERROR);
                            _this166.disconnect();
                        }
                    };
                }
            });
            _this166.websocket.onclose = function (evt) {
                _this166.websocket = null;
                if (_this166.retryDelay < 0) {
                    _this166.disconnecting = true;
                }
                if (_this166.disconnecting) {
                    return;
                }
                // connection error
                _this166.imm_testHubAgainLater();
            };
            _this166.websocket.onerror = function (evt) {
                console.log('WebSocket error: ', evt);
                if (_this166.websocket.terminate) {
                    _this166.websocket.terminate();
                }
                _this166.websocket = null;
                if (_this166.retryDelay < 0) {
                    _this166.disconnecting = true;
            ...
            return yield _this166.websocketOpenPromise;
        })();
    }
    ...
    _webSocketMsg(arr_bytes) {
        ...
        return _asyncToGenerator(function* () {
            try {
                let reltime = (_this167._yapi.GetTickCount() - _this167._connectionTime) / 1000.0;
                let ystream = arr_bytes[0] >>> 3;
                var text = '';
                if (ystream == _this167._YSTREAM_TCP_NOTIF) {
                    //console.log(reltime+': TCP_NOTIF len='+arr_bytes.length);
                    for (let i = 1; i < arr_bytes.length; i++) {
                        text += String.fromCharCode(arr_bytes[i]);
                    }
                    yield _this167._yapi.parseEvents(_this167, text);
                    return;
                }
                // Other types of messages
                var ws = _this167.websocket;
                var tcpchan = arr_bytes[0] & 7;
                if (ystream == _this167._YSTREAM_TCP || ystream == _this167._YSTREAM_TCP_CLOSE || ystream == _this167._YSTREAM_TCP_ASYNCCLOSE) {
                    if (tcpchan > 3) {
                        _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Unexpected frame for tcpChan ' + tcpchan);
                        return;
                    }
                    let tcp_end = arr_bytes.length;
                    let yreq = _this167.tcpChan[tcpchan];
                    //console.log(reltime+': TCP ystream='+ystream+' len='+arr_bytes.length);
                    if (!yreq) {
                        _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Drop frame for closed tcpChan ' + tcpchan);
                        return;
                    }
                    if (ystream == _this167._YSTREAM_TCP_ASYNCCLOSE) {
                        // async close packet, check async signature byte
                        tcp_end--;
                        if (yreq.asyncId == 0) {
                            _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Asynchronous close received, sync reply request');
                            return;
                        } else if (yreq.asyncId != arr_bytes[tcp_end]) {
                            _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Incorrect async-close signature on tcpChan ' + tcpchan);
                            return;
                        }
                    }
                    ...
                    if (ystream == _this167._YSTREAM_TCP_CLOSE || ystream == _this167._YSTREAM_TCP_ASYNCCLOSE) {
                        // Pop request from tcp channel
                        _this167.tcpChan[tcpchan] = yreq.next;

                        // Handle synchronous close
                        if (ystream == _this167._YSTREAM_TCP_CLOSE) {
                            // synchronous close
                            if (yreq.asyncId != 0) {
                                _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Synchronous close received, async ack expected');
                                return;
                            } else {
                                ...
                                if (yreq.timeoutId) {
                                    ...
                                    frame[0] = (_this167._YSTREAM_TCP_CLOSE << 3) + tcpchan;
                                    _this167.websocket.send(frame);
                                }
                            }
                        }
                        ...
                        if (pos < 0) {
                            ...
                            let header = _this167._yapi.imm_bin2str(yreq.bin_result.subarray(0, pos));
                        ...
                        if (yreq.asyncId == 0) {
                            ...
                            if (yreq.errorType != YAPI_SUCCESS) {
                                _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Async request error: ' + yreq.errorMsg);
                            }
                        }
                    }
                    return;
                }
                if (ystream == _this167._YSTREAM_META) {
                    let metatype = arr_bytes[1];
                    //console.log(reltime+': META type='+metatype+' len='+arr_bytes.length);
                    switch (metatype) {
                        case _this167._USB_META_WS_ANNOUNCE:
                            if (arr_bytes.length < 1 + _this167._USB_META_WS_ANNOUNCE_SIZE) {
                                return;
                            }
                            _this167._remoteVersion = arr_bytes[2];
                            if (_this167._remoteVersion < 1) {
                                return;
                            }
                            var maxtcpws = (arr_bytes[3] << 4) + (arr_bytes[4] << 12);
                            if (maxtcpws > 0) {
                                _this167._tcpMaxWindowSize = maxtcpws;
                            }
                            _this167._remoteNonce = arr_bytes[5] + (arr_bytes[6] << 8) + (arr_bytes[7] << 16) + (arr_bytes[8] << 24);
                            for (let i = 9; i < 9 + 20; i++) {
                                if (arr_bytes[i] == 0) {
                                    _this167._remoteSerial = _this167._yapi.imm_bin2str(arr_bytes.subarray(9, i));
                                    break;
                                }
                            }
                            _this167._nonce = parseInt(Math.random() * 0xffffffff);
                            _this167._connectionTime = _this167._yapi.GetTickCount();
                            _this167._connectionState = _this167._WS_AUTHENTICATING;
                            // send our authentication packet
                            let frame = new Uint8Array(1 + _this167._USB_META_WS_AUTHENTICATION_SIZE);
                            let version = _this167._remoteVersion < 2 ? _this167._remoteVersion : 2;
                            let flags = 0;
                            frame[0] = _this167._YSTREAM_META << 3;
                            frame[1] = _this167._USB_META_WS_AUTHENTICATION;
                            frame[2] = version;
                            if (_this167.urlInfo.pass != '') {
                                flags = _this167._USB_META_WS_VALID_SHA1;
                                let sha1 = _this167.imm_computeAuth(_this167.urlInfo.user, _this167.urlInfo.pass, _this167._remoteSerial, _this167._remoteNonce);
                            ...
                            frame[3] = flags & 0xff;
                            frame[4] = flags >>> 8;
                            frame[5] = _this167._nonce & 0xff;
                            frame[6] = _this167._nonce >>> 8 & 0xff;
                            frame[7] = _this167._nonce >>> 16 & 0xff;
                            frame[8] = _this167._nonce >>> 24 & 0xff;
                            _this167.websocket.send(frame);
                            break;
                        case _this167._USB_META_WS_AUTHENTICATION:
                            if (_this167._connectionState != _this167._WS_AUTHENTICATING) {
                                return;
                            }
                            if (arr_bytes.length < 1 + _this167._USB_META_WS_AUTHENTICATION_SIZE) {
                                return;
                            }
                            _this167._tcpRoundTripTime = _this167._yapi.GetTickCount() - _this167._connectionTime + 1;
                            if (_this167._tcpMaxWindowSize < 2048 && _this167._tcpRoundTripTime < 7) {
                                // Fix overly optimistic round-trip on YoctoHubs
                                _this167._tcpRoundTripTime = 7;
                            }
                            let uploadRate = parseInt(_this167._tcpMaxWindowSize * 1000 / _this167._tcpRoundTripTime);
                            ...
                            if ((inflags & _this167._USB_META_WS_RW) != 0) {
                                _this167._rwAccess = true;
                            }
                            if ((inflags & _this167._USB_META_WS_VALID_SHA1) != 0) {
                                let remote_sha1 = arr_bytes.subarray(9, 29);
                                let sha1 = _this167.imm_computeAuth(_this167.urlInfo.user, _this167.urlInfo.pass, _this167._remoteSerial, _this167._nonce);
                                for (let i = 0; i < sha1.length; i++) {
                                    if (sha1[i] != remote_sha1[i]) {
                                        // bad signature
                                        _this167._session_errno = 401;
                                        _this167._session_error = 'Authentication failed';
                                        _this167._connectionState = _this167._WS_DEAD;
                                        return;
                                    }
                                }
                                // Password verified OK
                                _this167._connectionState = _this167._WS_CONNECTED;
                            } else {
                                if (_this167.urlInfo.pass == '') {
                                    ...
                                    _this167._session_errno = 401;
                                    if (_this167.urlInfo.user == 'admin' && !_this167._rwAccess) {
                                        _this167._session_error = 'Authentication as admin failed';
                                    } else {
                                        _this167._session_error = 'Password not set on remote hub';
                                    }
                                    _this167._connectionState = _this167._WS_DEAD;
                                    return;
                                }
                            ...
                            break;
                        case _this167._USB_META_WS_ERROR:
                            _this167._session_errno = arr_bytes[3] + (arr_bytes[4] << 8);
                            if (_this167._session_errno == 401) {
                                _this167._session_error = 'Authentication failed';
                            } else {
                                _this167._session_error = 'Remote hub closed connection with error ' + _this167._session_errno;
                            }
                            _this167._connectionState = _this167._WS_DEAD;
                            break;
                        case _this167._USB_META_ACK_UPLOAD:
                            tcpchan = arr_bytes[2];
                            if (_this167.tcpChan[tcpchan]) {
                                let yreq = _this167.tcpChan[tcpchan];
                                let ackBytes = arr_bytes[3] + (arr_bytes[4] << 8) + (arr_bytes[5] << 16) + (arr_bytes[6] << 24);
                                let ackTime = _this167._yapi.GetTickCount();
                                if (_this167._lastUploadAckTime[tcpchan] != 0 && ackBytes > _this167._lastUploadAckBytes[tcpchan]) {
                                    _this167._lastUploadAckBytes[tcpchan] = ackBytes;
                                    _this167._lastUploadAckTime[tcpchan] = ackTime;
                                    let deltaBytes = ackBytes - _this167._lastUploadRateBytes[tcpchan];
                                    let deltaTime = ackTime - _this167._lastUploadRateTime[tcpchan];
                                    if (deltaTime < 500) break; // wait more
                                    if (deltaTime < 1000 && deltaBytes < 65536) break; // wait more
                                    _this167._lastUploadRateBytes[tcpchan] = ackBytes;
                                    _this167._lastUploadRateTime[tcpchan] = ackTime;
                                    if (yreq.progressCb && yreq.toBeSent) {
                                        yreq.progressCb(ackBytes, yreq.toBeSent.length);
                                    }
                                    let newRate = deltaBytes * 1000 / deltaTime;
                                    _this167._uploadRate[tcpchan] = parseInt(0.8 * _this167._uploadRate[tcpchan] + 0.3 * newRate); // +10% intentionally
                ...
                _this167.imm_asyncWebSocketError(YAPI_IO_ERROR, 'Unsupported message: ' + _this167._yapi.imm_bin2hexstr(arr_bytes));
            } catch (e) {
    ...
    request(method, devUrl, obj_body, tcpchan) {
        ...
        return _asyncToGenerator(function* () {
            if (_this168.logging) {
                _this168.imm_logrequest(method, devUrl, obj_body);
            }
            //noinspection UnnecessaryLocalVariableJS
            let httpPromise = new Promise(function (resolve, reject) {
                let subReq = method + ' ' + devUrl + ' \r\n\r\n';
                let ws = _this168.websocket;
                let isAsync = _this168._remoteVersion > 0 && devUrl.slice(-2) == '&.';
                ...
                yreq.sendPos = 0;
                if (obj_body) {
                    let boundary = _this168.imm_getBoundary();
                    let body = _this168.imm_formEncodeBody(obj_body, boundary);
                ...
                if (!ws || _this168.disconnecting || _this168._connectionState != _this168._WS_CONNECTED) {
                    yreq.errorType = YAPI_IO_ERROR;
                ...
                if (isAsync) {
                    yreq.asyncId = _this168.nextAsyncId++;
                    if (_this168.nextAsyncId >= 127) {
                        _this168.nextAsyncId = 48;
                    }
                }

                // Queue all requests on tcpChan 0 for now, to preserve request order
                let queue = _this168.tcpChan[tcpchan];
                if (queue) {
                    while (queue.next) {
                        queue = queue.next;
                    }
                    ...
                    _this168.tcpChan[tcpchan] = yreq;
                }

                // Send request if possible
                _this168.imm_sendPendingRequest(tcpchan);
            });
    ...
    imm_sendPendingRequest(tcpchan) {
        ...
        while (yreq) {
            ...
            if (end > 2108 && this._remoteVersion >= 2 && tcpchan == 0) {
                ...
                if (pos == 0) {
                    ...
                    yreq.sendTimeoutId = setTimeout(function () {
                        _this169.imm_sendPendingRequest(tcpchan);
            ...
            yreq.timeoutId = setTimeout(function (chan, yr) {
                _this169.imm_abortRequest(chan, yr);
    ...
    imm_abortRequest(tcpchan, yreq) {
        ...
        setTimeout(function (chan, yr) {
            _this170.imm_forgetRequest(chan, yr);
        }, 5000, tcpchan, yreq);
    ...
    disconnect() {
        ...
        return _asyncToGenerator(function* () {
            var tcpchan_busy;
            var timeout = _this171._yapi.GetTickCount() + 3000;
            do {
                tcpchan_busy = false;
                for (var tcpchan = 0; tcpchan < 4; tcpchan++) {
                    if (_this171.tcpChan[tcpchan] != null) {
                        tcpchan_busy = true;
                        break;
                    }
                }
                if (tcpchan_busy) {
                    yield _this171._yapi._microSleep_internal();
                }
            } while (tcpchan_busy && timeout > _this171._yapi.GetTickCount());

            _this171.imm_commonDisconnect();
            _this171.websocket.close();
            _this171.websocket = null;
        })();
    }
}
...
class YAPIContext {
    ...
    _addHub(newhub) {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this172._hubs.push(newhub);

            // If hub is not yet known, create a device object (synchronous call)
            var serial = _this172._snByUrl[newhub.urlInfo.url];
            if (!serial) {
                let dev = new YDevice(_this172, newhub.urlInfo.url, null, null);
                yield dev.refresh();
            }
        })();
    }
    ...
    _updateDeviceList_internal(bool_forceupdate, bool_invokecallbacks) {
        ...
        return _asyncToGenerator(function* () {
            if (_this173._firstArrival && bool_invokecallbacks && _this173._arrivalCallback) {
                bool_forceupdate = true;
            }
            if (bool_forceupdate) {
                for (let i = 0; i < _this173._hubs.length; i++) {
                    _this173._hubs[i].imm_forceUpdate();
                }
            }
            if (_this173._updateDevListStarted && _this173.GetTickCount() - _this173._updateDevListStarted < 30 * 1000) {
                return {
            ...
            try {
                ...
                _this173._updateDevListStarted = _this173.GetTickCount();

                // collect list of hubs which should be checked
                let hubs = [];
                for (let i = 0; i < _this173._hubs.length; i++) {
                    let hub = _this173._hubs[i];
                    let rootUrl = hub.urlInfo.url;
                    let hubDev = _this173.imm_getDevice(rootUrl);
                    if (!hubDev) continue;
                    if (hub.devListExpires <= _this173.GetTickCount()) {
                        hub.missing = [];
                        hubs.push(hub);
                    }
                }
                ...
                for (let serial in _this173._devs) {
                    let rooturl = _this173._devs[serial].imm_getRootUrl();
                    for (let i = 0; i < hubs.length; i++) {
                ...
                if (bool_invokecallbacks) {
                    let nbEvents = _this173._pendingCallbacks.length;
                    for (let i = 0; i < nbEvents; i++) {
                        let evt = _this173._pendingCallbacks[i];
                        let serial = evt.slice(1);
                        switch (evt.charAt(0)) {
                            case '+':
                                if (_this173._arrivalCallback != undefined) {
                                    yield _this173._arrivalCallback(yFindModule(serial + '.module'));
                                }
                                break;
                            case '/':
                                if (_this173._namechgCallback != undefined) {
                                    yield _this173._namechgCallback(yFindModule(serial + '.module'));
                                }
                                break;
                            case '-':
                                if (_this173._removalCallback != undefined) {
                                    yield _this173._removalCallback(yFindModule(serial + '.module'));
                                }
                                _this173.imm_forgetDevice(_this173._devs[serial]);
                                break;
                        }
                    }
                    _this173._pendingCallbacks = _this173._pendingCallbacks.slice(nbEvents);
                    if (_this173._arrivalCallback != undefined && _this173._firstArrival) {
                        _this173._firstArrival = false;
                    }
                }
            } finally {
                _this173._updateDevListStarted = false;
            }
    ...
    updateDeviceList_process(hub, hubDev, whitePages, yellowPages) {
        ...
        return _asyncToGenerator(function* () {
            ...
            for (let classname in yellowPages) {
                ...
                let ftype = _this174._fnByType[classname];
                if (ftype == undefined) {
                    ftype = new YFunctionType(_this174, classname);
                    _this174._fnByType[classname] = ftype;
                }
            ...
            for (let devkey in whitePages) {
                ...
                var currdev = _this174._devs[serial];
                if (currdev && _this174._arrivalCallback != undefined && _this174._firstArrival) {
                    _this174._pendingCallbacks.push('+' + serial);
                }
                hub.serialByYdx[devydx] = serial;
                if (!currdev) {
                    // Add new device
                    //noinspection ObjectAllocationIgnored
                    new YDevice(_this174, rooturl, devinfo, yellowPages);
                    if (_this174._arrivalCallback != undefined) {
                        _this174._pendingCallbacks.push('+' + serial);
                    }
                } else if (currdev.imm_getLogicalName() != devinfo['logicalName']) {
                    // Reindex device from its own data
                    yield currdev.refresh();
                    if (_this174._namechgCallback != undefined) {
                        _this174._pendingCallbacks.push('/' + serial);
                    }
            ...
            for (serial in hub.missing) {
                if (hub.missing[serial]) {
                    if (_this174._removalCallback != undefined) {
                        _this174._pendingCallbacks.push('-' + serial);
                    } else {
                        _this174.imm_forgetDevice(_this174._devs[serial]);
                    }
                }
            }
        ...
        return _asyncToGenerator(function* () {
            ...
            for (var idx = 0; idx < nrows; idx++) {
                ...
                if (ev.length >= 3 && firstCode >= NOTIFY_NETPKT_FLUSHV2YDX && firstCode <= NOTIFY_NETPKT_TIMEAVGYDX) {
                    ...
                    if (serial && _this175._devs[serial]) {
                        var funcid = funydx == 0xf ? 'time' : _this175._devs[serial].imm_functionId(funydx);
                        if (funcid != '') {
                            value = ev.slice(3);
                            switch (firstCode) {
                                case NOTIFY_NETPKT_FUNCVALYDX:
                                    ...
                                    yield _this175.setFunctionValue(serial + '.' + funcid, value);
                                    break;
                                ...
                                case NOTIFY_NETPKT_TIMEV2YDX:
                                    ...
                                    var dev = _this175._devs[serial];
                                    if (funcid == 'time') {
                                        var time = arr[1] + 0x100 * arr[2] + 0x10000 * arr[3] + 0x1000000 * arr[4];
                                        dev.imm_setDeviceTime(time + arr[5] / 250.0);
                                    } else {
                                        yield _this175.setTimedReport(serial + '.' + funcid, dev.imm_getDeviceTime(), arr);
                                    }
                                    break;
                                case NOTIFY_NETPKT_FUNCV2YDX:
                                    var rawval = _this175.imm_decodeNetFuncValV2(value);
                                    if (rawval != null) {
                                        var decodedval = _this175.imm_decodePubVal(rawval[0], rawval, 1, 6);
                                        yield _this175.setFunctionValue(serial + '.' + funcid, decodedval);
                                    }
                                    break;
                    ...
                    if (notype == '@') {
                        ...
                        case 5:
                            ...
                            if (parts.length > 2) {
                                ...
                                yield _this175.setFunctionValue(parts[0] + '.' + parts[1], value[0]);
                            }
                            break;
                    }
                } else {
            ...
            if (_this175._forwardValues > 0) {
                yield _this175.HandleEvents(new YErrorMsg());
            }
        })();
    }
    ...
    _UpdateValueCallbackList(obj_func, bool_add) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {number} **/
            let index = _this176._ValueCallbackList.indexOf(obj_func);
            if (bool_add) {
                yield obj_func.isOnline();
                if (index < 0) {
                    _this176._ValueCallbackList.push(obj_func);
                }
            } else if (index >= 0) {
                _this176._ValueCallbackList.splice(index, 1);
            }
        })();
    }
    ...
    _UpdateTimedReportCallbackList(obj_func, bool_add) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {number} **/
            let index = _this177._TimedReportCallbackList.indexOf(obj_func);
            if (bool_add) {
                yield obj_func.isOnline();
                if (index < 0) {
                    _this177._TimedReportCallbackList.push(obj_func);
                }
            } else if (index >= 0) {
                _this177._TimedReportCallbackList.splice(index, 1);
            }
        })();
    }
    ...
    setTimedReport(str_hwid, float_timestamp, arr_report) {
        var _this179 = this;

        return _asyncToGenerator(function* () {
            let classname = _this179.imm_functionClass(str_hwid);
            let receivers = _this179._TimedReportCallbackList;
            for (let i = 0; i < receivers.length; i++) {
                let fun = receivers[i];
                if (!fun._hwId) continue;
                if (fun._hwId == str_hwid) {
                    var dev = _this179.imm_getDevice(fun._serial);
                    if (dev) {
                        var report = yield fun._decodeTimedReport(float_timestamp, arr_report);
                        yield fun._invokeTimedReportCallback(report);
                    }
                }
            }
        })();
        ...
        return _asyncToGenerator(function* () {
            ...
            if (str_device.substr(0, 7) == 'http://' || str_device.substr(0, 5) == 'ws://') {
                ...
                if (lines[0].substr(0, 12) != 'GET /not.byn') {
                    /** @type {string} **/
                    let serial = _this180._snByUrl[baseUrl];
                    if (serial) {
                        lockdev = _this180._devs[serial];
                    }
                }
            } else {
                lockdev = _this180.imm_getDevice(str_device);
                if (!lockdev) {
                ...
                if (lines[0] == 'GET /api.json') {
                    return yield lockdev.requestAPI(_this180.defaultCacheValidity);
                }
            ...
            /** @type {YGenericHub} **/
            let hub = _this180.imm_getHub(baseUrl);
            /** @type {string} **/
            let method = words[0];
            /** @type {string} **/
            let devUrl = words[1];
    ...
    _funcDev(str_className, str_func) {
        ...
        return _asyncToGenerator(function* () {
            var resolve = _this181.imm_funcDev_internal(str_className, str_func);
            if (resolve.errorType == YAPI_SUCCESS) {
                return resolve;
            } else if (resolve.errorType == YAPI_DEVICE_NOT_FOUND && _this181._hubs.length == 0) {
            ...
            var updRes = yield _this181._updateDeviceList_internal(true, false);
            if (updRes.errorType != YAPI_SUCCESS) {
                resolve.errorType = updRes.errorType;
                resolve.errorMsg = updRes.errorMsg;
                return resolve;
            }
            return _this181.imm_funcDev_internal(str_className, str_func);
        })();
    }
    ...
    funcRequest(str_className, str_func, str_extra, int_msValidity) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YFuncRequest} **/
            let funcreq = _this182.imm_funcDev_internal(str_className, str_func);
            if (funcreq.errorType != YAPI_SUCCESS) {
                funcreq = yield _this182._funcDev(str_className, str_func);
                if (funcreq.errorType != YAPI_SUCCESS) {
                    return funcreq;
                }
            }
            ...
            if (str_extra == '') {
                ...
                if (yreq != null) {
                    if (yreq.errorType != YAPI_SUCCESS) return yreq;
                    try {
                        let jsonData = JSON.parse(_this182.imm_bin2str(yreq.bin_result));
            ...
            if (!loadval) {
                ...
                /** @type {YHTTPRequest} **/
                yreq = yield _this182.devRequest(devreq.deviceid, httpreq, null, 0);
                if (yreq.errorType != YAPI_SUCCESS) return yreq;
                ...
                try {
                    loadval = JSON.parse(_this182.imm_bin2str(yreq.bin_result));
                } catch (err) {
                    //console.log('RequestAPI parse error: ', err);
                }
            }
    ...
    HTTPRequest(str_device, str_request) {
        ...
        return _asyncToGenerator(function* () {
            /** @type {YHTTPRequest} **/
            let yreq = yield _this183.devRequest(str_device, str_request, null, 0);
            if (yreq.errorType != YAPI_SUCCESS) {
                return _this183._throw(yreq.errorType, yreq.errorMsg, null);
            }
            return yreq.bin_result;
        })();
    }
    ...
    GetAPIVersion() {
        ...
        return _asyncToGenerator(function* () {
            return _this184.imm_GetAPIVersion();
        })();
    }

    imm_GetAPIVersion() {
        return (/* version number patched automatically */'1.10.26380'
        );
    }
    ...
    FreeAPI() {
        ...
        return _asyncToGenerator(function* () {
            for (let serial in _this185._devs) {
                let lockdev = _this185._devs[serial];
            ...
            for (let i = 0; i < _this185._hubs.length; i++) {
                _this185._hubs[i].disconnect();
            }
    ...
    DisableExceptions() {
        ...
        return _asyncToGenerator(function* () {
            _this186.exceptionsDisabled = true;
        })();
    }
    ...
    EnableExceptions() {
        ...
        return _asyncToGenerator(function* () {
            _this187.exceptionsDisabled = false;
        })();
    }
    ...
    RegisterHub(url, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            let urlInfo = _this189.imm_parseRegisteredUrl(url);
            let newhub = _this189.imm_getHub(urlInfo.url);
            if (newhub) {
                return YAPI_SUCCESS;
            }
            newhub = _this189.imm_registerHub_internal(urlInfo);
            let retcode = yield newhub.testHub(errmsg);
            if (retcode != YAPI_SUCCESS) {
                return _this189._throw(retcode, errmsg.msg, retcode);
            }
            yield _this189._addHub(newhub);

            // Update known device list
            let yreq = yield _this189._updateDeviceList_internal(true, false);
            if (yreq.errorType != YAPI_SUCCESS) {
                if (errmsg != undefined) {
                    errmsg.msg = yreq.errorMsg;
                    _this189._serverResponse.write('\n!YoctoAPI:' + errmsg.msg + '\n');
                }
                return _this189._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    PreregisterHub(url, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            let urlInfo = _this190.imm_parseRegisteredUrl(url);
            let newhub = _this190.imm_getHub(urlInfo.url);
            if (newhub) {
                return YAPI_SUCCESS;
            }
            newhub = _this190.imm_registerHub_internal(urlInfo);
            _this190._addHub(newhub);
    ...
    RegisterHubHttpCallback(incomingMessage, serverResponse, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            let urlInfo = _this191.imm_parseRegisteredUrl('http://callback:4444');
            let newhub = _this191.imm_getHub(urlInfo.url);
            if (newhub) {
                return YAPI_SUCCESS;
            }
            newhub = new YHttpCallbackHub(_this191, urlInfo, incomingMessage, serverResponse);
            yield newhub.httpCallbackPromise;
            let retcode = yield newhub.testHub(errmsg);
            if (retcode != YAPI_SUCCESS) {
                _this191._serverResponse.write('\n!YoctoAPI:' + errmsg.msg + '\n');
                return _this191._throw(retcode, errmsg.msg, retcode);
            }
            yield _this191._addHub(newhub);

            // Update known device list
            let yreq = yield _this191._updateDeviceList_internal(true, false);
            if (yreq.errorType != YAPI_SUCCESS) {
                if (errmsg != undefined) {
                    errmsg.msg = yreq.errorMsg;
                    _this191._serverResponse.write('\n!YoctoAPI:' + errmsg.msg + '\n');
                }
                return _this191._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    RegisterHubWebSocketCallback(ws, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            let urlInfo = _this192.imm_parseRegisteredUrl('http://callback:4444');
            let newhub = _this192.imm_getHub(urlInfo.url);
            if (newhub) {
                return YAPI_SUCCESS;
            }
            newhub = new YWebSocketCallbackHub(_this192, urlInfo, ws);
            let retcode = yield newhub.testHub(errmsg);
            if (retcode != YAPI_SUCCESS) {
                return _this192._throw(retcode, errmsg.msg, retcode);
            }
            yield _this192._addHub(newhub);

            // Update known device list
            let yreq = yield _this192._updateDeviceList_internal(true, false);
            if (yreq.errorType != YAPI_SUCCESS) {
                if (errmsg != undefined) {
                    errmsg.msg = yreq.errorMsg;
                }
                return _this192._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    UnregisterHub(url) {
        ...
        return _asyncToGenerator(function* () {
            var urlInfo = _this193.imm_parseRegisteredUrl(url);
            var i, j;

            for (i = 0; i < _this193._hubs.length; i++) {
                if (_this193._hubs[i].urlInfo.url == urlInfo.url) {
                    _this193._hubs[i].disconnect();
                    for (j = 0; j < _this193._hubs[i].serialByYdx.length; j++) {
                        var serial = _this193._hubs[i].serialByYdx[j];
                        _this193.imm_forgetDevice(_this193._devs[serial]);
                    }
                    var before = _this193._hubs.slice(0, i);
                    if (i + 1 < _this193._hubs.length) {
                        var after = _this193._hubs.slice(i + 1);
                        _this193._hubs = before.concat(after);
                    }
                    _this193._hubs = before;
                    return;
                }
            }
        })();
     ...
     * On failure returns a negative error code.
     */
    TestHub(url, mstimeout) {
        var _this194 = this;

        return _asyncToGenerator(function* () {
            let urlInfo = _this194.imm_parseRegisteredUrl(url);
            let newhub = _this194.imm_getHub(urlInfo.url);
            if (newhub) {
                return YAPI_SUCCESS;
            }
            newhub = _this194.imm_registerHub_internal(urlInfo);
            let errmsg = new YErrorMsg();
            return yield newhub.testHub(errmsg);
        })();
    }
    ...
    UpdateDeviceList(errmsg) {
        ...
        return _asyncToGenerator(function* () {
            var yreq = yield _this195._updateDeviceList_internal(false, true);
            if (yreq.errorType != YAPI_SUCCESS) {
                if (errmsg != undefined) errmsg.msg = yreq.errorMsg;
                return _this195._throw(yreq.errorType, yreq.errorMsg, yreq.errorType);
            }
            return YAPI_SUCCESS;
        })();
    }
    ...
    Sleep(ms_duration, errmsg) {
        ...
        return _asyncToGenerator(function* () {
            var end = _this196.GetTickCount() + ms_duration;
            yield _this196.HandleEvents(errmsg);
            while (_this196.GetTickCount() < end) {
                yield _this196._microSleep_internal();
                yield _this196.HandleEvents(errmsg);
            }
            return YAPI_SUCCESS;
        })();
    ...
    _setTimeout_internal(callback, endtime, args, errmsg) {
        ...
        if (delay < 0) {
            ...
            this.Sleep(delay, errmsg).then(function () {
                _this197._setTimeout_internal(callback, endtime, args);
            });
        } else {
            this.UpdateDeviceList(errmsg).then(function () {
                _this197.Sleep(90, errmsg).then(function () {
                    _this197._setTimeout_internal(callback, endtime, args);
                });
            });
        }
    ...
    RegisterDeviceArrivalCallback(arrivalCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this198._arrivalCallback = arrivalCallback;
        })();
    }
    ...
    RegisterDeviceChangeCallback(changeCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this199._namechgCallback = changeCallback;
        })();
    }
    ...
    RegisterDeviceRemovalCallback(removalCallback) {
        ...
        return _asyncToGenerator(function* () {
            _this200._removalCallback = removalCallback;
        })();
    }
    ...
    RegisterCalibrationHandler(calibrationType, calibrationHandler) {
        ...
        return _asyncToGenerator(function* () {
            _this201._calibHandlers[calibrationType.toString()] = calibrationHandler;
        })();
    }
    ...
    ComputePSK(ssid, pass) {
        ...
        return _asyncToGenerator(function* () {
            ...
            _this202.imm_initshaw(pass, 0, 0, 0x3636, _shaw);
            _this202.imm_itershaw(sha1_init, _shaw);
            for (k = 0; k < 5; k++) inner[k] = _shaw[k];
            _shaw = new Uint32Array(80);
            _this202.imm_initshaw(pass, 0, 0, 0x5c5c, _shaw);
            _this202.imm_itershaw(sha1_init, _shaw);
            ...
            _shaw = new Uint32Array(80);
            _this202.imm_initshaw(ssid, 0, 1, 0, _shaw);

            for (iter = 0; iter < 8192;) {
                _this202.imm_itershaw(inner, _shaw);
                _shaw[5] = 0x80000000;
                for (k = 6; k < 15; k++) {
                    _shaw[k] = 0;
                }
                _shaw[15] = 8 * (64 + 20);
                _this202.imm_itershaw(outer, _shaw);
                shau[0] ^= _shaw[0];
                shau[1] ^= _shaw[1];
                shau[2] ^= _shaw[2];
                shau[3] ^= _shaw[3];
                ...
                if ((iter & 4095) == 0) {
                    ...
                    if (iter == 4096) {
                        for (k = 0; k < 5; k++) shau[k] = 0;
                        _shaw = new Uint32Array(80);
                        _this202.imm_initshaw(ssid, 0, 2, 0, _shaw);
                    }
                }
            }
            var hex = '';
