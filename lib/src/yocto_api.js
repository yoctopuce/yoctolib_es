...
export class YFunction
{
    ...
    async get_syncProxy()
    {
        ...
        return new YFunctionProxy(this);
    }
...
export class YWebSocketHub extends YGenericHub
{
    ...
    async testHub(errmsg)
    {
        if (this.disconnecting || !this.websocket) {
            return YAPI_IO_ERROR;
        }
    ...
    async _webSocketMsg(arr_bytes)
    {
        ...
        if(ystream == this._YSTREAM_TCP || ystream == this._YSTREAM_TCP_CLOSE || ystream == this._YSTREAM_TCP_ASYNCCLOSE) {
            ...
            if(ystream == this._YSTREAM_TCP_ASYNCCLOSE) {
                ...
                tcp_end--;
                if(this.tcpChan[tcpchan].asyncId != arr_bytes[tcp_end]) {
