...
export class YFirmwareUpdate
{
    ...
    async _processMore(newupdate)
    {
        ...
        /** @type {YFirmwareFile} **/
        let firmware = YFirmwareFile.imm_Parse(this._firmwarepath, bytes);
        ...
        await hub.firmwareUpdate(this._serial, firmware, this._settings, (percent, msg) => {
            this.imm_progress(5 + percent * 80 / 100, msg);
        });
    ...
    static async checkFirmware_r(file, serial_base)
    {
        if(YAPI._isNodeJS) {
    ...
    static async CheckFirmware(serial,path,minrelease)
    {
        ...
        if (typeof path == 'string' && path.indexOf('yoctopuce.com') >= 0) {
            ...
            /** @type {YFirmwareFile} **/
            let firmware = await YFirmwareUpdate.checkFirmware_r(path, serial.substring(0, 8));
            if (firmware != null) {
...
export class YFunction
{
    ...
    async _upload(str_path, bin_content)
    {
        ...
        return await this._yapi.devRequest(devid, httpreq, new YHTTPBody(str_path, bin_content));
    }
...
{   
    ...
    async firmwareUpdate(serial, firmware, settings, progress)
    {
        ...
        yreq = await this.request('POST', baseUrl + '/upload.html', new YHTTPBody('firmware', firmware.imm_getData()), 0);
        if(yreq.errorType != YAPI_SUCCESS) {
        ...
        if (use_self_flash) {
            ...
            yreq = await this.request('POST', baseUrl + '/upload.html', new YHTTPBody('startupConf.json', settings), 0);
            if(yreq.errorType != YAPI_SUCCESS) {
                return _throw('Failed to save settings on hub');
            }
            yreq = await this.request('POST', baseUrl + '/upload.html', new YHTTPBody('firmwareConf', settings), 0);
            if(yreq.errorType != YAPI_SUCCESS) {
                return _throw('Failed to save settings on hub');
            }
        }
    ...
    async _webSocketMsg(arr_bytes)
    {
        try {
            ...
            if(ystream == this._YSTREAM_TCP || ystream == this._YSTREAM_TCP_CLOSE || ystream == this._YSTREAM_TCP_ASYNCCLOSE) {
                ...
                if(!yreq) {
                    this.imm_asyncWebSocketError('Drop frame for closed tcpChan '+tcpchan);
                    return;
                }
                if(ystream == this._YSTREAM_TCP_ASYNCCLOSE) {
                    ...
                    tcp_end--;
                    if(yreq.asyncId == 0) {
                        this.imm_asyncWebSocketError('Asynchronous close received, sync reply request');
                        return;
                    } else if(yreq.asyncId != arr_bytes[tcp_end]) {
                        this.imm_asyncWebSocketError('Incorrect async-close signature on tcpChan '+tcpchan);
                        return;
                    }
                }
                ...
                if(ystream == this._YSTREAM_TCP_CLOSE || ystream == this._YSTREAM_TCP_ASYNCCLOSE) {
                    ...
                    if(ystream == this._YSTREAM_TCP_CLOSE) {
                        // synchronous close
                        if(yreq.asyncId != 0) {
                            this.imm_asyncWebSocketError('Synchronous close received, async ack expected');
                            return;
                    ...
                    if(yreq.asyncId == 0) {
                        ...
                        if(yreq.errorType != YAPI_SUCCESS) {
                            this.imm_asyncWebSocketError('Async request error: '+yreq.errorMsg);
                        }
                    }
                }
                return;
            ...
            this.imm_asyncWebSocketError('Unsupported message: '+this._yapi.imm_bin2hexstr(arr_bytes));
        } catch(e) {
