...
class YTemperature extends _yocto_api.YSensor {
    ...
    get_syncProxy() {
        ...
        return _asyncToGenerator(function* () {
            ...
            return new YTemperatureProxy(_this);
        })();
    }
    ...
    set_ntcParameters(res25, beta) {
        ...
        return _asyncToGenerator(function* () {
            ...
            return yield _this7.set_thermistorResponseTable(tempValues, resValues);
        })();
    }
    ...
    set_thermistorResponseTable(tempValues, resValues) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (!(siz >= 2)) {
                return _this8._throw(_this8._yapi.INVALID_ARGUMENT, 'thermistor response table must have at least two points', _this8._yapi.INVALID_ARGUMENT);
            }
            if (!(siz == resValues.length)) {
                return _this8._throw(_this8._yapi.INVALID_ARGUMENT, 'table sizes mismatch', _this8._yapi.INVALID_ARGUMENT);
            }
            // may throw an exception
            res = yield _this8.set_command('Z');
            if (!(res == _this8._yapi.SUCCESS)) {
                return _this8._throw(_this8._yapi.IO_ERROR, 'unable to reset thermistor parameters', _this8._yapi.IO_ERROR);
            }
            ...
            while (found > 0) {
                ...
                if (found > 0) {
                    res = yield _this8.set_command('m' + String(Math.round(Math.round(1000 * curr))) + ':' + String(Math.round(Math.round(1000 * currTemp))));
                    if (!(res == _this8._yapi.SUCCESS)) {
                        return _this8._throw(_this8._yapi.IO_ERROR, 'unable to reset thermistor parameters', _this8._yapi.IO_ERROR);
                    }
                    prev = curr;
    ...
    loadThermistorResponseTable(tempValues, resValues) {
        ...
        return _asyncToGenerator(function* () {
            ...
            resValues.length = 0;
            // may throw an exception
            id = yield _this9.get_functionId();
            id = id.substr(11, id.length - 11);
            bin_json = yield _this9._download('extra.json?page=' + id);
            paramlist = _this9.imm_json_get_array(bin_json);
